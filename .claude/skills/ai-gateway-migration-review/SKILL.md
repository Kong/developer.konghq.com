...
Plugins have been replaced by AI Policies in v2. Some proxy plugins are exceptions, and they fall into two groups:

**Still exist as plugins (no policy equivalent) — flag references for manual review:**
- AI A2A Proxy
- AI MCP Proxy

**Removed entirely in v2 — do not exist as plugins *or* policies:**
- AI Proxy
- AI Proxy Advanced

The functionality of AI Proxy and AI Proxy Advanced (LLM proxying, routing, translation, and load balancing) is now handled by the {{site.ai_gateway}} core through the [AI Model](/ai-gateway/entities/ai-model/) and [AI Provider](/ai-gateway/entities/ai-provider/) entities. Since these plugins no longer exist, references to them must **always** be reframed in terms of v2 concepts (see the AI Proxy reframing rule below) — never left as valid plugin references.
...
- **Exception — flag these**: Any reference to AI A2A Proxy or AI MCP Proxy as plugins should be flagged for manual review (these still exist as plugins but have no policy equivalent). Do **not** apply this to AI Proxy or AI Proxy Advanced — those are removed entirely and must be reframed, not flagged (see the next rule).

- **AI Proxy / AI Proxy Advanced → reframe to v2 concepts (they no longer exist)**: AI Proxy and AI Proxy Advanced have been removed entirely in v2 — there is no such plugin or policy. The LLM proxying, routing, translation, and load balancing they performed is now handled by the {{site.ai_gateway}} core through the [AI Model](/ai-gateway/entities/ai-model/) entity (backed by an [AI Provider](/ai-gateway/entities/ai-provider/)). An **AI Model is essentially the v2 abstraction of an AI Proxy plugin instance**. Because these plugins no longer exist, every reference must be reframed — never left as a valid plugin reference. Reframe by what the sentence is actually about:

  - **Proxying / routing step** (e.g. "runs after the AI Proxy plugin"): reframe in terms of {{site.ai_gateway}} proxying to the upstream LLM service through an AI Model.
    - "The AI Response Transformer Policy runs **after** the AI Proxy plugin, and **after** proxying to the upstream service…"
      → "The AI Response Transformer Policy runs **after** {{site.ai_gateway}} proxies to the upstream LLM service through an [AI Model](/ai-gateway/entities/ai-model/)…"
  - **Supported providers** (e.g. "the same providers as the AI Proxy plugin"): point at the canonical AI Providers list.
    - "…supports `llm/v1/chat` requests for the same LLM providers as the [AI Proxy plugin](/plugins/ai-proxy/)."
      → "…supports `llm/v1/chat` requests for the same [LLM providers](/ai-gateway/ai-providers/) that {{site.ai_gateway}} supports."
  - **Shared config / tuning parameters** (e.g. "the same `config.llm` block as the AI Proxy plugin"): attribute the config to the AI Model.
    - "It also uses the same configuration and tuning parameters as the AI Proxy plugin, in the `config.llm` block."
      → "It also uses the same LLM configuration and tuning parameters as an [AI Model](/ai-gateway/entities/ai-model/), in the `config.llm` block."

  If a specific AI Proxy reference doesn't fit any of these shapes and the correct v2 phrasing is genuinely unclear, flag it for manual review rather than guessing — but the default is to reframe, since the plugin no longer exists.
