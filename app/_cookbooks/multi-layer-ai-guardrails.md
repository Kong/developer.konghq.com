...
prereqs:
  ...
  inline:
    ...
    - title: PII Anonymizer Service
      content: |
        The [AI PII Sanitizer](/plugins/ai-sanitizer/) Plugin requires an external PII anonymizer service to detect and replace sensitive data. Kong provides Docker images on a private Cloudsmith registry, with one image per supported NLP language model.
        ...
        1. Start the PII anonymizer service. The image tag selects the language model. For example, `:v0.1.4-en` for English or `:v0.1.4-es` for Spanish. See the [available language tags](/plugins/ai-sanitizer/#available-language-tags) for the full list. Each model needs at least 600 MB of memory:
...
2. The AI PII Sanitizer Plugin sends the prompt content to the PII anonymizer service, which scans for names, phone numbers, emails, credit card numbers, SSNs, and other sensitive types. Detected PII is replaced with placeholder tokens like `PLACEHOLDER1` and `PLACEHOLDER2` before the request continues. If the PII service is unreachable and `stop_on_error` is `true`, the request is rejected rather than forwarded with unsanitized content.
...
**`anonymize: [all_and_credentials]`**, enables every supported PII recognizer the anonymizer service ships with: names, phone numbers, emails, credit cards, SSNs, passports, IPs, credentials, and more. To narrow the scope to specific types (for example, only `email` and `ssn`), replace the list with just those entries. For the full enum, see the [AI PII Sanitizer](/plugins/ai-sanitizer/) reference.
...
**Adjust regex and semantic thresholds.** The semantic guard's `0.75` default is tuned for OpenAI's `text-embedding-3-large`; lower it to require closer matches, raise it to catch broader variations, and retune whenever you change embedding models (different models use different distance scales). For regex patterns, add domain-specific terms and use `allow_patterns` to allowlist legitimate terms that contain blocked substrings (for example, "hackathon").
...
**Use Kong Vaults for production credential management.** Replace the environment variable exports with vault references to store your LLM API keys, Redis credentials, and PII service host securely. Kong supports HashiCorp Vault, AWS Secrets Manager, GCP Secret Manager, and the Konnect Config Store. See the [secrets management documentation](/gateway/secrets-management/) for setup instructions.
...
Stop and remove the Redis and PII anonymizer containers if you started them locally:
