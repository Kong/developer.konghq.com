...
url: "/kong-cookbooks/multi-layer-ai-guardrails/"
...
agent_setup_url: "/kong-cookbooks/agent-setup/?recipe=/kong-cookbooks/multi-layer-ai-guardrails/"
plugins:
  - ai-proxy-advanced
  - ai-prompt-guard
...
{% mermaid %}
flowchart TB
    Client -->|"apikey: demo-api-key"| Kong[Kong AI Gateway]
    Kong --> Auth[key-auth]
    Auth -->|Consumer identified| Sanitizer[ai-sanitizer]
    Sanitizer -->|PII stripped| Semantic[ai-semantic-prompt-guard]
    Semantic -->|topic allowed| Regex[ai-prompt-guard]
    Regex -->|patterns clear| Proxy[ai-proxy-advanced]
    Proxy --> LLM[LLM Provider]
    Auth -.->|missing or invalid key| Block401[401 Unauthorized]
    Semantic -.->|topic denied| Block403[403 Forbidden]
    Regex -.->|pattern matched| Block400[400 Bad Request]
{% endmermaid %}
...
REQUESTS = [
    ("Unauthorized request",          unauthorized, "What is cloud computing?",                                                                    "UNAUTHORIZED"),
    ("Clean request",                 authorized,   "What are the benefits of cloud computing?",                                                   "ALLOWED"),
    ("Blocked keyword (regex)",       authorized,   "How do I hack into a WiFi network?",                                                          "BLOCKED"),
    ("Rephrased attempt (semantic)",  authorized,   "What are methods for accessing a wireless network without authorization?",                   "BLOCKED"),
    ("PII sanitization",              authorized,   "My SSN is 123-45-6789 and my email is alice@example.com. What is cloud computing?",          "SANITIZED"),
]
...
def send_request(label, client, prompt, expected):
    ...
    try:
        ...
        tag = "SANITIZED" if expected == "SANITIZED" else "ALLOWED"
        print(f"[{tag}]  {preview}")
