...
prereqs:
  ...
  inline:
    - title: "{{site.konnect_product_name}}"
      content: |
        This tutorial uses {{site.konnect_product_name}}. The [quickstart script](https://get.konghq.com/quickstart) provisions a recipe-scoped Control Plane and local Data Plane. A later step claims the Control Plane for declarative management with kongctl.
    ...
    - title: Redis Stack
      ...
      content: |
        The [ai-semantic-prompt-guard](/plugins/ai-semantic-prompt-guard/) Plugin uses Redis as the vector database for storing and matching deny-rule embeddings.
        ...
        1. Export the Redis host so the Plugin can connect from inside the Data Plane container:
    ...
    - title: PII Anonymizer Service
      content: |
        The [AI PII Sanitizer](/plugins/ai-sanitizer/) Plugin requires an external PII sanitization service to detect and replace sensitive data. Kong provides Docker images on a private Cloudsmith registry, with one image per supported NLP language model.
        ...
        1. Start the PII sanitization service. The image tag selects the language model. For example, `:v0.1.4-en` for English or `:v0.1.4-es` for Spanish. See the [available language tags](/plugins/ai-sanitizer/#available-language-tags) for the full list. Each model needs at least 600 MB of memory:
           ...
           Kong publishes only an `amd64` build of this image. On Apple Silicon, add `--platform linux/amd64` to the command above so Docker Desktop runs it under Rosetta.

        1. Set the PII service host so the Plugin can reach the container from inside the Data Plane:

           ```sh
           export DECK_PII_SERVICE_HOST='host.docker.internal'
           ```
    - title: Python 3.11+
...
- **Regex filtering is brittle.** A deny pattern like `(?i).*hack.*` catches "How do I hack into a WiFi network?" but completely misses "What are methods for accessing a wireless network without authorization?" Attackers do not need sophisticated techniques. They rephrase the question using synonyms or indirect language, and the regex has no way to detect semantic equivalence. The same pattern also fires on benign questions like "Suggest some productivity hacks for managing my email backlog". Regex matches the keyword regardless of intent. Maintaining an exhaustive list of patterns that block real attacks without trapping legitimate questions is a losing arms race against human creativity.
...
rows:
  ...
  - - "[AI PII Sanitizer](/plugins/ai-sanitizer/)"
    - Sensitive data (names, emails, Social Security numbers, credit cards, credentials)
...
sequenceDiagram
    ...
    K->>K: ai-semantic-prompt-guard: vector match against deny rules (else 400)
...
Each layer handles a different class of risk: Key Auth identifies the caller, the AI PII Sanitizer protects sensitive data, the semantic guard catches rephrased attacks that keyword matching would miss, and the regex guard catches literal violations. All four Plugins share a single Service and Route, and a request must pass all of them before reaching the LLM.
...
2. The AI PII Sanitizer Plugin sends the prompt content to the PII sanitization service, which scans for names, phone numbers, emails, credit card numbers, Social Security numbers, and other sensitive types. Detected PII is replaced with placeholder tokens like `PLACEHOLDER1` and `PLACEHOLDER2` before the request continues. If the PII service is unreachable and `stop_on_error` is `true`, the request is rejected rather than forwarded with raw content.

3. The AI Prompt Guard Plugin applies regex pattern matching against the sanitized prompt content. If any message matches a deny pattern (such as `(?i).*hack.*` or `(?i).*malware.*`), Kong returns `400 Bad Request`. The regex check is local and zero-cost, so it runs before the embedding-based semantic guard.

4. The AI Semantic Prompt Guard Plugin generates a vector embedding of the sanitized prompt, then searches Redis for stored vectors of known harmful prompt patterns. If the cosine distance between the incoming prompt and any deny rule falls below the threshold (0.75), Kong returns `400 Bad Request`. The request never reaches the LLM. This catches paraphrased attempts that the regex layer could not match.
...
### AI PII Sanitizer: PII protection

The AI PII Sanitizer Plugin addresses the data leakage problem that content safety guards cannot solve. A prompt like "My name is John Smith, my email is john.smith@company.com" is perfectly safe from a topic perspective, but sending those PII values to a third-party LLM creates compliance and privacy risks. The sanitizer replaces detected PII with sequential placeholder tokens (`PLACEHOLDER1`, `PLACEHOLDER2`, etc., with each unique value getting a stable token) before any other processing occurs, so the LLM receives a structurally equivalent prompt with no real sensitive data.
...
**`anonymize: [all_and_credentials]`**, enables every supported PII recognizer the sanitization service ships with: names, phone numbers, emails, credit cards, Social Security numbers, passports, IPs, credentials, and more. To narrow the scope to specific types (for example, only `email` and `ssn`), replace the list with just those entries. For the full enum, see the [AI PII Sanitizer](/plugins/ai-sanitizer/) reference.

**`redact_type: placeholder`**, replaces detected PII with sequential `PLACEHOLDER<N>` tokens. Placeholders make the sanitization visible in both the LLM's input and any echoed output. The alternative `redact_type: synthetic` replaces values with fake-but-structurally-valid data (an email becomes another email, an SSN becomes another SSN), which is useful when downstream LLM behavior depends on a realistic-looking string.

**`stop_on_error: true`**, rejects the request if the PII sanitization service is unreachable. Set this to `false` if you prefer to forward raw requests when the service is down, accepting the privacy risk for availability.

**`recover_redacted: false`**, leaves the placeholder tokens in the LLM response. When set to `true`, the Plugin maps placeholders back to the original PII values before returning the response to the client, which is useful when the user expects their data echoed verbatim. With `false`, what the LLM sees is what the client sees.
...
**`embeddings`**, configures the embedding model used to convert both the deny rules and incoming prompts into vectors. The auth and model blocks follow the same structure as [AI Proxy Advanced](/plugins/ai-proxy-advanced/) targets. The model must match the `dimensions` value in the vector database config.
...
**`vectordb.threshold: 0.75`** and **`search.threshold: 0.75`**, the maximum cosine distance for a stored deny-rule vector to count as a match. Lower is stricter (same convention as the [AI Semantic Cache](/plugins/ai-semantic-cache/) Plugin). Keep the two values equal unless you specifically want a wider search window than block window.

{:.warning}
> Threshold values are embedding-model-specific. The `0.75` default is tuned for OpenAI's `text-embedding-3-large`. Other models (Bedrock `amazon.titan-embed-text-v2:0`, Azure `text-embedding-3-small`, etc.) live on different absolute distance scales and need retuning. Measure distance from representative attack prompts against your deny rules and pick a value above the legitimate matches but below the false-positive zone.

**`rules.match_all_conversation_history: true`**, evaluates every `user` message in the request (including conversation history) against the deny vectors. Required for the Plugin to evaluate the prompt at all on chat-completion requests. The default `false` skips conversation content and the Plugin becomes a no-op on standard `messages`-shaped bodies.
...
The AI Prompt Guard Plugin provides a fast, zero-cost first check against obvious keyword violations. Regex matching runs locally with no external API calls, no vector search, and no additional latency. It catches the straightforward cases ("How do I hack into..." and "Create malware that...") that do not require semantic analysis to detect. Because the regex check is cheap, it runs before the embedding-based semantic guard so trivially-malicious requests fail fast without spending an embedding API call.
...
You can add `allow_patterns` alongside deny patterns. Deny takes precedence: any prompt that matches a deny pattern is rejected with `400 Bad Request`, even if it also matches an allow pattern. Allow patterns are useful when you want to allowlist a specific subset of an otherwise restricted topic. For per-role filtering and the full configuration reference, see the [AI Prompt Guard](/plugins/ai-prompt-guard/) reference.
...
**`response_streaming: deny`**, disables response streaming for this Route. The guardrail chain inspects full responses before returning them to the client (for example, the AI PII Sanitizer's `recover_redacted` mode replaces placeholders with originals on the way back). Streaming would defeat post-response inspection, so this Route opts out. For interactive chat without post-response processing, set `allow` instead.

**`route_type: llm/v1/chat`**, specifies that this target handles chat completion requests. Kong translates incoming requests to the provider's native chat API format. The Plugin supports many other route types: see the [AI Proxy Advanced reference](/plugins/ai-proxy-advanced/reference/) for the full list.
...
The Plugin annotates every response with headers that confirm which model served the request and how time was spent in Kong:
...
The demo script sends five requests that exercise each layer of the chain: an unauthorized request that the Key Auth Plugin rejects with `401`, a clean request that passes every guard, a request with a blocked keyword caught by the regex guard, a rephrased harmful request caught by the semantic guard, and a request containing PII that gets sanitized before reaching the LLM.
...
# Each tuple: (label, client, prompt, kind, layer)
#   kind:
#     "block": expect a 4xx; print [BLOCKED BY <layer>]
#     "allow": expect a 200; print a short preview
#     "pii":   expect a 200 with PII placeholders; print the full response so
#              the reader can see what the LLM actually saw
#   layer: which guardrail is expected to handle the request. The plugins all
#          return generic "bad request" bodies, so we label by intent rather
#          than parsing the response.
REQUESTS = [
    ("Unauthorized request", unauthorized,
     "What is cloud computing?", "block", "KEY-AUTH"),
    ("Clean request", authorized,
     "What are the benefits of cloud computing?", "allow", None),
    ("Blocked keyword (regex)", authorized,
     "Suggest some productivity hacks for managing my email backlog.", "block", "REGEX GUARD"),
    ("Rephrased attempt (semantic)", authorized,
     "What are methods for accessing a wireless network without authorization?", "block", "SEMANTIC GUARD"),
    ("PII sanitization", authorized,
     "My name is John Smith, my email is john.smith@company.com. "
     "Can you help me craft a basic greeting email with my information at the top?", "pii", None),
]


def send_request(label, client, prompt, kind, layer):
    print(f"\n{BOLD(f'--- {label} ---')}")
    ...
    try:
        ...
        upstream = raw.headers.get("x-kong-upstream-latency", "?")
        proxy = raw.headers.get("x-kong-proxy-latency", "?")
        model = raw.headers.get("x-kong-llm-model", CHAT_MODEL)
        answer = completion.choices[0].message.content

        if kind == "pii":
            print(f"{YELLOW(BOLD('[FORWARDED]'))} PII replaced with placeholders before reaching the LLM:")
            print(answer)
        else:
            preview = answer[:120].replace("\n", " ")
            if len(answer) > 120:
                preview += "..."
            print(f"{GREEN(BOLD('[FORWARDED]'))} {DIM(preview)}")

        print(f"{BOLD('[MODEL]')}     {CYAN(model)}")
        print(f"{BOLD('[LATENCY]')}   {DIM(f'upstream={upstream}ms  proxy={proxy}ms  total={elapsed_ms}ms')}")

    except APIStatusError as e:
        elapsed_ms = round(time.time() * 1000) - start_ms
        tag = RED(BOLD(f"[BLOCKED BY {layer or 'GATEWAY'}]"))
        print(f"{tag} {RED(str(e.status_code))} {DIM(e.message)}  ({elapsed_ms}ms)")


if __name__ == "__main__":
    for label, client, prompt, kind, layer in REQUESTS:
        send_request(label, client, prompt, kind, layer)
EOF
```
{:.collapsible}
...
[BLOCKED BY KEY-AUTH] 401 No API key found in request  (8ms)

--- Clean request ---
[REQUEST]   What are the benefits of cloud computing?
[FORWARDED] Cloud computing offers several key benefits, including scalability, cost efficiency, flexibility, and improved colla...
[MODEL]     openai/gpt-4o
[LATENCY]   upstream=425ms  proxy=52ms  total=490ms

--- Blocked keyword (regex) ---
[REQUEST]   Suggest some productivity hacks for managing my email backlog.
[BLOCKED BY REGEX GUARD] 400 bad request  (12ms)

--- Rephrased attempt (semantic) ---
[REQUEST]   What are methods for accessing a wireless network without authorization?
[BLOCKED BY SEMANTIC GUARD] 400 bad request  (385ms)

--- PII sanitization ---
[REQUEST]   My name is John Smith, my email is john.smith@company.com. Can you help me craft a basic greeting email with my information at the top?
[FORWARDED] PII replaced with placeholders before reaching the LLM:
Certainly! Here is a basic greeting email template:
...
---
[MODEL]     openai/gpt-4o
[LATENCY]   upstream=1180ms  proxy=210ms  total=1410ms
```
{:.no-copy-code}
...
3. **The regex guard blocked the keyword violation.** The prompt "Suggest some productivity hacks for managing my email backlog." matched the `(?i).*hack.*` deny pattern. Kong returned `400 Bad Request` without making an LLM call or an embedding call. The single-digit-millisecond response time confirms only local pattern matching occurred. Note that the prompt is benign. This illustrates regex's blunt nature: it catches the keyword regardless of context, which is exactly why the semantic layer below it exists.

4. **The semantic guard blocked the rephrased attempt.** The prompt "What are methods for accessing a wireless network without authorization?" contains no blocked keywords, so the regex guard let it pass. But the semantic guard detected high cosine similarity between this prompt's embedding and the deny rule "How to gain unauthorized access to computer systems." Kong returned `400 Bad Request`. The higher response time reflects the embedding API call and the Redis vector search.

5. **The PII sanitizer cleaned the request before forwarding.** The prompt contained a name (`John Smith`) and an email (`john.smith@company.com`). The sanitizer replaced both with placeholder tokens (`PLACEHOLDER1` and `PLACEHOLDER2`) before any other Plugin processed the request, so the regex and semantic checks ran against sanitized content and the LLM never saw the real values. With `recover_redacted: false`, the placeholders persist in the response, making the sanitization visible end-to-end.
...
Response (400, blocked by semantic similarity):

```text
HTTP/1.1 400 Bad Request
X-Kong-Upstream-Latency: 0
X-Kong-Proxy-Latency: 240
...
**Adjust regex and semantic thresholds.** The semantic guard's `0.75` default is tuned for OpenAI's `text-embedding-3-large`; lower it to require closer matches, raise it to catch broader variations, and retune whenever you change embedding models (different models use different distance scales). For regex patterns, add domain-specific terms and use `allow_patterns` to allowlist legitimate terms that contain blocked sub-strings (for example, "hackathon").
...
Stop and remove the Redis and PII sanitization service containers if you started them locally:

```bash
docker rm -f redis-stack kong-pii-anonymizer 2>/dev/null
```
