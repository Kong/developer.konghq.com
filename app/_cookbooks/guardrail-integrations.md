...
`KONNECT_CONTROL_PLANE_NAME` and `KONNECT_TOKEN` are exported once during the {{site.konnect_product_name}} prerequisites, and `DECK_OPENAI_TOKEN`, `DECK_AZURE_CONTENT_SAFETY_*`, and `DECK_MISTRAL_MODERATION_TOKEN` come from the credential prerequisites. Each tab below exports only the model selection that varies per configuration.
