...
url: "/kong-cookbooks/claude-code-sso/"
...
try:
    ...
    print(f"[MODEL] {kong_model}")
    print(f"[LATENCY] upstream={upstream_ms}ms  proxy={proxy_ms}ms  total={elapsed_ms}ms")
    print(f"[TOKEN LIMIT] {remaining or 'n/a'}/{limit or 'n/a'} remaining (60s window)")
...
for i in range(1, 6):
    try:
        ...
        remaining, limit = get_rate_limit_headers(raw.headers)
        print(f"[REQUEST {i}] tokens remaining={remaining or 'n/a'}/{limit or 'n/a'}")
    except APIStatusError as e:
        if e.status_code == 429:
            retry_after = e.response.headers.get('retry-after', 'n/a')
            print(f"[REQUEST {i}] TOKEN LIMIT EXCEEDED, retry after {retry_after}s")
            break
        print(f"[REQUEST {i}] {e.status_code} {e.message}")

# --- 3. Invalid JWT request ---
print("\n=== Invalid JWT request ===")
print("[REQUEST] Authorization: Bearer not-a-valid-jwt")
...
try:
    ...
    print(f"[ERROR] {e.status_code} {reason}")
EOF
```
{:.collapsible}
