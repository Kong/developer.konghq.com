...
url: "/kong-cookbooks/secure-external-mcp-gateway/"
...
agent_setup_url: "/kong-cookbooks/agent-setup/?recipe=/kong-cookbooks/secure-external-mcp-gateway/"
plugins:
  - ai-mcp-proxy
  - ai-mcp-oauth2
  - datakit
...
prereqs:
  ...
  inline:
    ...
    - title: Identity Provider
      content: |
        The Konnect MCP pattern uses the [AI MCP OAuth2](/plugins/ai-mcp-oauth2/) Plugin with your organization's IdP. This requires the same IdP setup as the [Secure Internal MCP Gateway](/kong-cookbooks/secure-internal-mcp-gateway/) recipe. If you've already completed that setup, you can reuse the same IdP applications.

        {% navtabs "Identity Provider" %}
        {% tab Okta %}

        Create two Okta applications as described in the [Secure Internal MCP Gateway prerequisites](/kong-cookbooks/secure-internal-mcp-gateway/#identity-provider):
        ...
        Create a Keycloak realm and two clients as described in the [Secure Internal MCP Gateway prerequisites](/kong-cookbooks/secure-internal-mcp-gateway/#identity-provider):
...
overview: |
  This recipe is the companion to [Secure Internal MCP Gateway](/kong-cookbooks/secure-internal-mcp-gateway/),
  ...
  - **Internal MCP servers** are hosted within your trust boundary. You control both the server
    ...
    See [Secure Internal MCP Gateway](/kong-cookbooks/secure-internal-mcp-gateway/).
...
```text
MCP Client                     Kong Gateway                       GitHub
──────────                    ─────────────                      ──────

  │                           ┌─ /github-mcp ─────────────────────────────┐
  ├── MCP init ─────────────► │   datakit                                 │
  │   (no token)              │     • No Authorization header?            │
  │                           │     • Return 401 + PRM pointer            │
  │                           │                                           │
  ├── GET PRM ──────────────► │   /.well-known/.../github-mcp             │
  │                           │     • Proxies GitHub's PRM                │
  │                           │     • Rewrites resource → Kong URL        │
  │                           │                                           │
  ├── OAuth flow ───────────► │   (browser → GitHub OAuth → callback)     │
  │                           │                                           │
  ├── MCP request ──────────► │   key-auth                                │
  │   apikey: dev-api-key     │     • Resolves Kong consumer              │
  │   Bearer: <github-token>  │                                           │
  │                           │   ai-mcp-proxy (passthrough-listener)     │
  │                           │     • Evaluates tool ACLs         GitHub  │
  │                           │     • Passes Bearer to upstream ─► MCP    │
  │                           │     • Logs tool calls              Server │
  │                           └───────────────────────────────────────────┘
```
{:.no-copy-code}
...
```text
MCP Client                     Kong Gateway                       Konnect
──────────                    ─────────────                      ───────

  │                           ┌─ /konnect-mcp ───────────────────────────┐
  ├── MCP init ─────────────► │   ai-mcp-oauth2                          │
  │   (no token)              │     • Returns 401 + PRM                   │
  │                           │                                           │
  ├── OAuth flow ───────────► │   (browser → Okta/Keycloak → callback)   │
  │                           │                                           │
  ├── MCP request ──────────► │   ai-mcp-oauth2                          │
  │   Bearer: <okta-token>    │     • Introspects token via IdP          │
  │                           │     • Maps sub → consumer                │
  │                           │     • Strips user token           Konnect │
  │                           │   request-transformer-advanced    MCP     │
  │                           │     • Injects stored SAT ────────► Server │
  │                           │   ai-mcp-proxy (passthrough-listener)     │
...
- **Combining with internal MCP.** Use the [Secure Internal MCP Gateway](/kong-cookbooks/secure-internal-mcp-gateway/)
