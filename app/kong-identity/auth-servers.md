...
{{site.identity}} generates both the `client_id` and the `client_secret` values when you create a client on the authorization server from the UI or send a POST request to the [API endpoint](/api/konnect/kong-identity/v1/#/operations/createAuthServerClient).
...
This allows you to [rotate secrets](/how-to/rotate-auth-server-client-secrets/) without incurring downtime using the API by sending a POST request to the `/auth-servers/{authServerId}/clients/{clientId}/secrets` endpoint.

Secret rotation is useful in cases such as:

- **Scheduled rotation:** When compliance policies require rotating credentials on a fixed schedule.
- **Leaked secret:** If a secret is exposed, you need to replace it without breaking any service that uses it.
- **Shared client:** When multiple services share one client, keeping both secrets active during rollout avoids breaking the ones that haven't switched yet.

When creating a new secret, you can either:

- Provide your own secret value in the `"secret"` field.
- Let {{site.identity}} generate the secret automatically and save the value returned in the API response.

#### API request examples

The following query examples show how to request the [{{site.identity}} Auth Servers API](/api/konnect/kong-identity/v1/) to create a new client secret with the following path parameters:

- Auth server ID
- Client ID

{% navtabs "create a new client secret" %}
{% navtab "Specify the new secret" %}
<!--vale off-->
{% konnect_api_request %}
url: /v1/auth-servers/{authServerId}/clients/{clientId}/secrets
status_code: 200
method: POST
headers:
...
<!--vale on-->
{% endnavtab %}
{% navtab "Secret generated by {{site.identity}}" %}
<!--vale off-->
{% konnect_api_request %}
url: /v1/auth-servers/{authServerId}/clients/{clientId}/secrets
status_code: 200
