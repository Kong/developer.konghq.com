...
The client credentials flow is a [standard OAuth flow](https://datatracker.ietf.org/doc/html/rfc6749#section-4.4) for using pre-determined credentials (such as a `client_id` and secret) for authenticating a client to {{site.identity}}. This is typically used in situations where the authentication context is _only the client itself_, and for this reason is often used for Machine-to-Machine (M2M) use cases, such as a microservice authenticating to {{site.base_gateway}}.
...
The specific client credentials that {{site.identity}} supports are the `client_id` (which is determined when the client is created on the auth server) and `client_secret`.
