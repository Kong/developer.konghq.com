...
{{site.identity}} generates both the `client_id` and the `client_secret` values when you create a client on the authorization server from the UI or send a POST request to the [API endpoint](/api/konnect/kong-identity/v1/#/operations/createAuthServerClient). To set them yourself, make a  PUT request to the [`clients` endpoint](/api/konnect/kong-identity/v1/#/operations/replaceAuthServerClient)
