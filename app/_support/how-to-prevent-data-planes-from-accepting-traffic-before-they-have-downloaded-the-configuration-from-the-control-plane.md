...
Previously, before `/status/ready` was available, a workaround was to create a custom `/health` route with a request-termination plugin returning 200, and point the readiness probe at that route on the proxy port instead. That approach still works, but it has a real downside compared to `/status/ready`: every readiness check is a proxy request and counts towards your overall request quota. Since `/status/ready` runs on the status port and is purpose-built for this check, the custom `/health` workaround is very likely unnecessary on current Kong Gateway versions.
