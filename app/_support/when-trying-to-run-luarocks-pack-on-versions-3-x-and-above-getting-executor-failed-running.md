...
tldr:
  ...
  a: |
    When installing custom plugins after upgrading to 3.x, `luarocks pack` fails because the `zip`
    tool is not installed. Run the command with `--verbose` to confirm: the logs will show
    `fs.is_tool_available("zip", "zip")` and `fs.search_in_path("zip")`, indicating `zip` is needed
    to handle the files. Install it in the Dockerfile with `RUN yum install zip -y`, then rerun and
    the `luarocks pack` command will succeed.
related_resources: []
---
