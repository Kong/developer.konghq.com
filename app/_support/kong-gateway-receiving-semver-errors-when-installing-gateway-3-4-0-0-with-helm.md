...
description: "Explains the Helm SemVer validation error (`Invalid Semantic Version`) that occurs when the chart's `image.tag` value doesn't match the required version format for Enterprise or open source Kong Gateway images."
products:
  - gateway
works_on:
  - on-prem
