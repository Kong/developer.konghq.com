---
title: "Debug {{site.base_gateway}} requests"
...
description: "Collect timing information about a given request."
...
Request debugging is enabled by default and has the following configurations in [`kong.conf`](/gateway/configuration/#request-debugging-section):
...
* **Filesystem:** The debug token is also stored in a file at `/.request_debug_token` and updated when {{site.base_gateway}} starts, restarts, or reloads.
...
* If the requests originate from anywhere other than the loopback addresses, you also need the `X-Kong-Request-Debug-Token` header.
...
curl http://localhost:8000/example \
    ...
    -H "X-Kong-Request-Debug-Token: $TOKEN"
```
