...
faqs:
  - q: What is the difference between a Vault and a Keyring?
    a: |
      ...
      You may choose a Vault if you want to secure certain fields that are not encrypted by the Keyring, or if you want to encrypt values in {{site.konnect_short_name}}. If all the fields you want to secure are encrypted and you're only running on-prem {{site.base_gateway}}, the Keyring may be a quicker solution to implement.
...
{:.info}
> For wider stability, security, and more granular control over encrypted values, we recommend using [Vaults](/gateway/entities/vault/) and database encryption at rest for [secret management](/gateway/secrets-management/):
> * Whenever possible, use Vaults for any secret values. Vaults are supported in {{site.konnect_short_name}}, have less performance impact on your {{site.base_gateway}} deployment, and support encrypting a wider range of values than Keyring.
> * Use database encryption at rest for your PostgreSQL database. Cloud vendors such as {{ site.amazon }} RDS normally enable this by default.
