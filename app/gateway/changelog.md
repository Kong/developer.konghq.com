...
* [**Request Callout**](/plugins/request-callout/) (`request-callout`)
...
* [**Session**](/plugins/session/) (`session`)
...
* [**AI Proxy**](/plugins/ai-proxy/) (`ai-proxy`) and [**AI Proxy Advanced**](/plugins/ai-proxy-advanced/) (`ai-proxy-advanced`)
...
#### Clustering
* Added support for [incremental config sync](https://docs.konghq.com/gateway/3.10.x/production/deployment-topologies/hybrid-mode/incremental-config-sync/) for hybrid mode deployments. 
...
You can use this setting to [strengthen security in Kong Manager](https://docs.konghq.com/gateway/3.10.x/kong-manager/strengthen-security/).
...
* Added a new core entity to Kong Gateway: [partials](https://docs.konghq.com/gateway/3.10.x/kong-enterprise/partials/). Partials enable users to define shared configuration for Redis.
...
* [**AI RAG Injector**](/plugins/ai-rag-injector/) (`ai-rag-injector`) Added the AI Rag Injector plugin, which allows automatically injecting documents to simplify building RAG pipelines.
* [**AI Sanitizer**](/plugins/ai-sanitizer/) (`ai-sanitizer`) Added the AI Sanitizer plugin, which can sanitize the PII information in requests before the requests are proxied by the AI Proxy or AI Proxy Advanced plugins.
* [**Confluent Consume**](/plugins/confluent-consume/) (`confluent-consume`): Introduced the Confluent Consume plugin, which adds Confluent Cloud consumption capabilities to Kong Gateway.
* [**Kafka Consume**](/plugins/kafka-consume/) (`kafka-consume`): Introduced the Kafka Consume plugin, which adds Kafka consumption capabilities to Kong Gateway.
* [**Request Callout**](/plugins/request-callout/) (`request-callout`): Added the `request-callout` plugin, which provides complex request augmentation and internal authentication.
...
* [**AI Proxy Advanced**](/plugins/ai-proxy-advanced/) (`ai-proxy-advanced`)
...
* [**AI Rate Limiting Advanced**](/plugins/ai-rate-limiting-advanced/) (`ai-rate-limiting-advanced`)  
...
* [**Confluent**](/plugins/confluent/) (`confluent`): 
...
* [**Kafka Upstream**](/plugins/kafka-upstream/) (`kafka-upstream`)
  * Added support for sending messages to multiple topics with `topics_query_arg`, and enabled topic allowlisting with `allowed_topics`.
  * Added support for message manipulation with the new configuration field `message_by_lua_functions`.

* [**JSON Threat Protection**](/plugins/json-threat-protection/) (`json-threat-protection`)
...
* [**AI Semantic Cache**](/plugins/ai-semantic-cache/) (`ai-semantic-cache`)  
...
* [**AI Semantic Prompt Guard**](/plugins/ai-semantic-prompt-guard/) (`ai-semantic-prompt-guard`)
  * Fixed an issue where Kong Gateway was not able to reconfigure the plugin when using DB-less mode.

* [**AWS Lambda**](/plugins/aws-lambda) (`aws-lambda`)
...
* [**gRPC-Web**](/plugins/grpc-web/) (`grpc-web`) and [**gRPC-Gateway**](/plugins/grpc-gateway/) (`grpc-gateway`)
...
* [**Service Protection**](/plugins/service-protection/) (`service-protection`)
  * Fixed an issue where the runtime failed due to `sync_rate` not being set if the `strategy` was `local`.
...
* [**Pre-function**](/plugins/pre-function/) (`pre-function`)
...
**New plugins:**
* [**Redirect**](/plugins/redirect/) (`redirect`): Introduced the Redirect plugin, which lets you redirect requests to another location.
 [#13900](https://github.com/Kong/kong/issues/13900)
* [**Injection Protection**](/plugins/injection-protection/) (`injection-protection`): Added the Injection Protection plugin, which supports blocking requests based on regex patterns.
* [**Service Protection**](/plugins/service-protection/) (`service-protection`): 
...
* [**AI Semantic Cache**](/plugins/ai-semantic-cache/) (`ai-semantic-cache`), [**AI Semantic Prompt Guard**](/plugins/ai-semantic-prompt-guard/) (`ai-semantic-prompt-guard`), [**AI Proxy Advanced**](/plugins/ai-proxy-advanced/) (`ai-proxy-advanced`)
  * Made the `embeddings.model.name` config field a free text entry, enabling use of a
self-hosted (or otherwise compatible) model.

* [**Correlation ID**](/plugins/correlation-id/) (`correlation-id`)
...
* [**AI Prompt Guard**](/plugins/ai-prompt-guard/) (`ai-prompt-guard`)
...
* [**AI Request Transformer**](/plugins/ai-request-transformer/) (`ai-request-transformer`) and 
[**AI Response Transformer**](/plugins/ai-response-transformer/) (`ai-response-transformer`)
...
* [**AI Semantic Prompt Guard**](/plugins/ai-semantic-cache/) (`ai-semantic-prompt-guard`)
  * Fixed an issue where the plugin could fail when handling requests with multiple models.
  * Fixed an issue where stale plugin config was not updated in DB-less and hybrid modes.
...
* [**DeGraphQL**](/plugins/degraphql/) (`degraphql`)
...
* [**Exit Transformer**](/plugins/exit-transformer/) (`exit-transformer`)
  * Fixed an issue where the plugin couldn't take effect on invalid non-admin requests.

* [**GraphQL Rate Limiting Advanced**](/plugins/graphql-rate-limiting-advanced/) (`graphql-rate-limiting-advanced`)
  * Fixed an issue where the plugin could fail to authenticate to Redis correctly with vault-referenced Redis configuration.
...
* [**JWE Decrypt**](/plugins/jwe-decrypt/) (`jwe-decrypt`)
...
* [**Kafka Log**](/plugins/kafka-log/) (`kafka-log`)
...
* [**Rate Limiting**](/plugins/rate-limiting/) (`rate-limiting`)
...
* [**JSON Threat Protection**](/plugins/json-threat-protection/)
  * Fixed an issue where the length counting of escape sequences, non-ASCII characters, and object entry names in JSON strings was incorrect. The plugin now uses UTF-8 character count instead of bytes.
...
* The following parameters have been deprecated:
  ...
  * The `timeout` config field in Redis configuration has been deprecated and replaced with `connect_timeout`, `send_timeout`, and `read_timeout`. 
  ...
  For more information about the Redis standardization changes, see the [3.8 Breaking Changes](https://docs.konghq.com/gateway/3.8.x/breaking-changes/).
...
response `Via` header if it is present in the [`headers`](https://docs.konghq.com/gateway/3.8.x/reference/configuration/#headers) config of `kong.conf`, in the format `2 kong/3.8.0`.
...
* Kong Gateway 3.8.x adds a new DNS client library. 
  This library is disabled by default, and can be enabled by setting the [`new_dns_client`](https://docs.konghq.com/gateway/3.8.x/reference/configuration/#new_dns_client) parameter to `on`.
  ...
  * Simplified and standardized logic.
  Learn more about enabling and using the new DNS client library in the [DNS migration guide](https://docs.konghq.com/gateway/3.8.x/migrate-to-new-dns-client/).
...
* Added connection support via Redis Proxy (for example, Envoy Redis proxy or twemproxy) via the configuration field [`connection_is_proxied`](https://docs.konghq.com/gateway/3.8.x/reference/configuration/#connection_is_proxied).
...
See the [PostgreSQL settings section](https://docs.konghq.com/gateway/3.8.x/reference/configuration/#postgres-settings) in the Kong configuration reference for details.
* Added keyring encryption support to [license database entity payloads](https://docs.konghq.com/gateway/3.8.x/kong-enterprise/db-encryption/#configure-license-payload-encryption).
...
See the [PostgreSQL settings section](https://docs.konghq.com/gateway/3.8.x/reference/configuration/#postgres-settings) in the Kong configuration reference for details.
* Added support for a configurable STS endpoint for AWS Vault. This can either be configured by [`vault_aws_sts_endpoint_url`](https://docs.konghq.com/gateway/3.8.x/reference/configuration/#vault_aws_sts_endpoint_url) as a global configuration, or [`sts_endpoint_url`](https://docs.konghq.com/gateway/3.8.x/kong-enterprise/secrets-management/backends/aws-sm/) on a custom AWS vault entity.
...
**New plugins**:
* [**AI Proxy Advanced**](/plugins/ai-proxy-advanced/): An advanced AI Proxy which supports load balancing between LLM services.
* [**AI Semantic Cache**](/plugins/ai-semantic-cache/): Configure an embeddings-based caching system for LLM responses.
* [**AI Semantic Prompt Guard**](/plugins/ai-semantic-prompt-guard/): Use semantic similarity-based prompt guarding with the AI Proxy.
* [**Upstream OAuth**](/plugins/upstream-oauth/): A plugin that enables Kong to obtain OAuth2 tokens to consume upstream APIs.
* [**Confluent**](/plugins/confluent/): Transform requests into Kafka messages in a Confluent topic.
* [**Standard Webhooks**](/plugins/standard-webhooks/): Validate that incoming webhooks adhere to the [Standard Webhooks](https://github.com/standard-webhooks/standard-webhooks)
 specification.
* [**Header Cert Authentication**](/plugins/header-cert-auth/): Authenticate clients with mTLS certificates passed in headers by a WAF or load balancer.
* [**JSON Threat Protection**](/plugins/json-threat-protection/): Validate JSON nesting depth, array elements, object entries, 
...
* [**GraphQL Proxy Cache Advanced**](/plugins/graphql-proxy-cache-advanced/) (`graphql-proxy-cache-advanced`),
[**GraphQL Rate Limiting Advanced**](/plugins/graphql-rate-limiting-advanced/) (`graphql-rate-limiting-advanced`), 
[**Proxy Cache Advanced**](/plugins/proxy-cache-advanced/) (`proxy-cache-advanced`), and 
[**Rate Limiting Advanced**](/plugins/rate-limiting-advanced/) (`rate-limiting-advanced`)
  * Added the Redis `cluster_max_redirections` configuration option.

* [**Response Transformer**](/plugins/response-transformer/) (`response-transformer`)
...
* [**gRPC-Gateway**](/plugins/grpc-gateway/) (`grpc-gateway`)
...
* [**HMAC Authentication**](/plugins/hmac-auth/) (`hmac-auth`), [**JWT**](/plugins/jwt/) (`jwt`), [**LDAP Authentication**](/plugins/ldap-auth/) (`ldap-auth`), and [**OAuth2**](/plugins/oauth2/) (`oauth2`)
...
* [**HTTP Log**](/plugins/http-log/) (`http-log`)
...
* [**Request Transformer**](/plugins/request-transformer/) (`request-transformer`)
...
* [**Basic Auth**](/plugins/basic-auth/) (`basic-auth`)
...
* [**Key Auth**](/plugins/key-auth/) (`key-auth`)
  * Fixed an issue where the realm field wasn't recognized for older Kong Gateway versions (earlier than 3.7).
   [#13042](https://github.com/Kong/kong/issues/13042)

* [**Request Size Limiting**](/plugins/request-size-limiting/) (`request-size-limiting`)
...
* [**Proxy Cache**](/plugins/proxy-cache/) (`proxy-cache`)
...
* [**OAuth 2.0 Introspection**](/plugins/oauth2-introspection/) (`oauth2-introspection`)
...
* [**Key Authentication Encrypted**](/plugins/key-auth-enc/) (`key-auth-enc`)
...
* [**Basic Authentication**](/plugins/basic-auth/) (`basic-auth`)
...
* [**AI Proxy**](/plugins/ai-proxy/) (`ai-proxy`): To support the new messages API of Anthropic,
...
* The **Granular Tracing** feature has been deprecated and removed.
  ...
  * `generate_trace_details`
  ...
  We recommend transitioning to [OpenTelemetry Instrumentation](https://docs.konghq.com/gateway/latest/production/tracing/) instead.
...
* Introduced the [`nginx_wasm_main_shm_kv`](https://docs.konghq.com/gateway/latest/reference/configuration/#nginx_wasm_main_shm_kv) configuration parameter, 
...
* Added the [`wasm_filters`](https://docs.konghq.com/gateway/latest/reference/configuration/#wasm_filters) configuration parameter for enabling individual filters.
...
[`router_flavor`](https://docs.konghq.com/gateway/latest/reference/configuration/#router_flavor) is 
`expressions`: `methods`, `hosts`, `paths`, `headers`,
`snis`, `sources`, `destinations`, and `regex_priority`.
The meaning of these fields are consistent with the [traditional route entity](https://docs.konghq.com/gateway/api/admin-ee/latest/#/Routes).
...
* [**AI Azure Content Safety**](/plugins/ai-azure-content-safety/) (`ai-azure-content-safety`): 
...
* [**OpenTelemetry**](/plugins/opentelemetry/) (`opentelemetry`) and [**Zipkin**](/plugins/zipkin/) (`zipkin`)
...
[**WebSocket Validator**](/plugins/websocket-validator/) (`websocket-validator`),
 [**XML Threat Protection**](/plugins/xml-threat-protection/) (`xml-threat-protection`)
  * The [priorities](https://docs.konghq.com/gateway/latest/plugin-development/custom-logic/#plugins-execution-order) of these plugins have been updated to prevent collisions between plugins.
...
* [LDAP Auth Advanced](/plugins/ldap-auth-advanced/) (`ldap-auth-advanced`)
  * Fixed an issue where an exception would be thrown when LDAP search failed.
...
* [**ACME**](/plugins/acme/) (`acme`), [**Rate Limiting**](/plugins/rate-limiting/) (`rate-limiting`), and 
[**Response Rate Limiting**](/plugins/response-ratelimiting/) (`response-ratelimiting`)
  * Fixed migration of Redis configuration.
...
If you still need to support TLS 1.1, set the [`ssl_cipher_suite`](https://docs.konghq.com/gateway/latest/reference/configuration/#ssl_cipher_suite) setting to `old`.
...
configuration parameters are set in `admin_gui_auth_conf` (like `session_secret`).
   ...
   See the [migration guide](https://docs.konghq.com/gateway/3.6.x/kong-manager/auth/oidc/migrate/) for more information.
...
* Added the `ngx_brotli` module to kong prebuild nginx.
  See the [documentation](https://docs.konghq.com/gateway/latest/production/performance/brotli/) to learn how to enable Brotli compression for Kong Gateway.
...
* Kong Gateway now supports [`http.path.segments.len` and `http.path.segments.*`](https://docs.konghq.com/gateway/latest/key-concepts/routes/expressions/#matching-fields)
...
Build provenance can be [verified by cosign/slsa-verifier](https://docs.konghq.com/gateway/3.6.x/kong-enterprise/provenance-verification/) 
using the published attestations.

#### Kong Manager Enterprise

* You can now use an RBAC token to authenticate while using 
[group mapping with Kong Manager](https://docs.konghq.com/gateway/3.6.x/kong-manager/auth/oidc/mapping/) (for example, with OIDC or LDAP).
...
**New plugins**:
* [**AI Proxy**](/plugins/ai-proxy/) (`ai-proxy`): Enables simplified integration with various AI provider Large Language Models (LLMs).
 [#12323](https://github.com/Kong/kong/issues/12323)
* [**AI Prompt Decorator**](/plugins/ai-prompt-decorator/) (`ai-prompt-decorator`): Prepend and append `llm/v1/chat` messages onto consumer LLM requests for prompt tuning.
 [#12336](https://github.com/Kong/kong/issues/12336)
* [**AI Prompt Guard**](/plugins/ai-prompt-guard/) (`ai-prompt-guard`): Set up allow or block lists for LLM requests based on pattern matching.
 [#12427](https://github.com/Kong/kong/issues/12427)
* [**AI Prompt Template**](/plugins/ai-prompt-template/) (`ai-prompt-template`): Set up an array of LLM prompt templates with variable substitutions.
 [#12340](https://github.com/Kong/kong/issues/12340)
* [**AI Request Transformer**](/plugins/ai-request-transformer/) (`ai-request-transformer`): Pass mid-flight client requests to an LLM for transformation or sanitization.
 [#12426](https://github.com/Kong/kong/issues/12426)
* [**AI Response Transformer**](/plugins/ai-response-transformer/) (`ai-response-transformer`): Pass mid-flight upstream responses to an LLM for transformation or sanitization.
 [#12426](https://github.com/Kong/kong/issues/12426)

Learn more about these plugins in the [AI Gateway quickstart](https://docs.konghq.com/gateway/latest/get-started/ai-gateway/).
...
* [**Datadog**](/plugins/datadog/) (`datadog`)
...
* [**Route Transformer Advanced**](/plugins/route-transformer-advanced/) (`route-transformer-advanced`)
...
* The recent OpenResty bump includes TLS 1.3 and deprecates TLS 1.1. 
If you still need to still support TLS 1.1, set the [`ssl_cipher_suite`](https://docs.konghq.com/gateway/latest/reference/configuration/#ssl_cipher_suite) setting to `old`.
...
* In OpenSSL 3.2, the default SSL/TLS security level has been changed from 1 to 2.
  ...
  Additionally, compression is disabled.
* The recent OpenResty bump includes TLS 1.3 and deprecates TLS 1.1. 
If you still need to support TLS 1.1, set the [`ssl_cipher_suite`](https://docs.konghq.com/gateway/3.5.x/reference/configuration/#ssl_cipher_suite) setting to `old`.
...
* You can now use an RBAC token to authenticate while using 
[group mapping with Kong Manager](https://docs.konghq.com/gateway/3.5.x/kong-manager/auth/oidc/mapping/) (for example, with OIDC or LDAP).
* Expressions router:
  *  The expressions router now supports the `!` (not) operator, which allows creating routes like
`!(http.path =^ "/a")` and `!(http.path == "/a" || http.path == "/b")`.
  * Kong Gateway now supports [`http.path.segments.len` and `http.path.segments.*`](https://docs.konghq.com/gateway/3.5.x/key-concepts/routes/expressions/#matching-fields)
  fields in the expressions router, which allows matching incoming (normalized) request paths by individual 
  segments or ranges of segments, and checking the total number of segments.
  * The [`net.src.*` and `net.dst.*`](https://docs.konghq.com/gateway/3.5.x/key-concepts/routes/expressions/#matching-fields) 
...
* [**mTLS Auth**](/plugins/mtls-auth/) (`mtls-auth`)
...
* [**DeGraphQL**](/plugins/degraphql/) (`degraphql`)
  * Fixed an issue where GraphQL variables were not being correctly parsed and coerced into their defined types.
* [**Rate Limiting**](/plugins/rate-limiting/) (`rate-limiting`), [**Rate Limiting Advanced**](/plugins/rate-limiting-advanced/) (`rate-limiting-advanced`), [**GraphQL Rate Limiting Advanced**](/plugins/graphql-rate-limiting-advanced/) (`graphql-rate-limiting-advanced`), and [**Response Rate Limiting**](/plugins/response-ratelimiting/) (`response-ratelimiting`)
  * Fixed an issue where any plugins using the `rate-limiting` library, when used together, 
  would interfere with each other and fail to synchronize counter data to the central data store.
...
### Features
#### Configuration
* The default value of the [`dns_no_sync`](https://docs.konghq.com/gateway/3.4.x/reference/configuration/#dns_no_sync) option has been changed to `off`.
...
* [**Session**](/plugins/session/) plugin: Introduced the new configuration field `read_body_for_logout` with a default value of `false`. 
...
Kong Konnect users can take advantage of our [API Analytics](https://docs.konghq.com/konnect/analytics/) offering, which provides a superset of Vitals functionality. 
...
* The default value of the [`dns_no_sync`](https://docs.konghq.com/gateway/3.5.x/reference/configuration/#dns_no_sync) option has been changed to `on`.
...
Set it up using the [`vault_azure_*`](https://docs.konghq.com/gateway/3.5.x/reference/configuration/#vault_azure_vault_uri).
...
For more information, see [Delete a workspace](https://docs.konghq.com/gateway/3.5.x/kong-manager/workspaces/#delete-a-workspace).
...
* Added the [`analytics_debug`](https://docs.konghq.com/gateway/3.5.x/reference/configuration/#analytics_debug)
 option to the output of logged requests. 
* Added the [`cluster_fallback_export_s3_config`](https://docs.konghq.com/gateway/3.5.x/reference/configuration/#cluster_fallback_export_s3_config) option to allow adding a 
...
* Introduced the new endpoint [`/schemas/vaults/:name`](https://docs.konghq.com/gateway/api/admin-ee/latest/#/operations/get-schemas-vaults-vault_name) for retrieving the schema of a vault. 
[#11727](https://github.com/Kong/kong/pull/11727)
* Renamed `privileged_agent` to [`dedicated_config_processing`](https://docs.konghq.com/gateway/3.5.x/reference/configuration/#dedicated_config_processing) and enabled `dedicated_config_processing` by default.
...
* Debugging tools:
  * Added a unique Request ID that is now populated in the error log, access log, error templates, log serializer, and a new `X-Kong-Request-Id` header. 
  ...
  [`headers`](https://docs.konghq.com/gateway/3.5.x/reference/configuration/#headers) and 
  [`headers_upstream`](https://docs.konghq.com/gateway/3.5.x/reference/configuration/#headers_upstream) configuration options. 
  ...
  * Added support for the debug request header `X-Kong-Request-Debug-Output`, 
  ...
  Enable it using the 
  [`request_debug`](https://docs.konghq.com/gateway/3.5.x/reference/configuration/#request_debug) configuration parameter.
  This header helps you diagnose the cause of any latency in Kong Gateway.
  See the [Request Debugging](https://docs.konghq.com/gateway/3.5.x/production/debug-request/) guide for more information.
...
Learn more about this function in the guide for [Implementing Custom Logic](https://docs.konghq.com/gateway/3.5.x/plugin-development/custom-logic/) for plugins.
...
* WebAssembly (Wasm):
  ...
  * Improved support for JSON in Wasm filter configuration.
  ...
  See the [Proxy-Wasm filter configuration](https://docs.konghq.com/gateway/3.5.x/plugin-development/wasm/filter-configuration/)
  guide to learn more.
...
* [TCP Log](/plugins/tcp-log) (`tcp-log`)
...
* Kong Gateway Enterprise container images are now produced with build provenance and signed using cosign. 
Signatures and attestations are published to the Docker Hub repository. 
Build provenance can be [verified by cosign/slsa-verifier](https://docs.konghq.com/gateway/3.4.x/kong-enterprise/provenance-verification/) 
using the published attestations.
...
* The recent OpenResty bump includes TLS 1.3 and deprecates TLS 1.1. 
If you still need to support TLS 1.1, set the [`ssl_cipher_suite`](https://docs.konghq.com/gateway/3.4.x/reference/configuration/#ssl_cipher_suite) setting to `old`.
...
* Added support for Microsoft Azure's KeyVault Secrets Engine. 
Set it up using the [`vault_azure_*`](https://docs.konghq.com/gateway/3.4.x/reference/configuration/#vault_azure_vault_uri)
configuration parameters.
* TLSv1.1 and lower is now disabled by default in OpenSSL 3.x.
...
* Added support for the debug request header `X-Kong-Request-Debug-Output`, which lets you observe the time consumed by specific components in a given request. Enable it using the [`request_debug`](https://docs.konghq.com/gateway/3.4.x/reference/configuration/#request_debug) configuration parameter. This header helps you diagnose the cause of any latency in Kong Gateway. See the [Request Debugging](https://docs.konghq.com/gateway/3.4.x/production/debug-request/) guide for more information.
* Kong Gateway now supports [`http.path.segments.len` and `http.path.segments.*`](https://docs.konghq.com/gateway/3.4.x/key-concepts/routes/expressions/#matching-fields) fields in the expressions router, which allows matching incoming (normalized) request paths by individual segments or ranges of segments, and checking the total number of segments.
* The [`net.src.*` and `net.dst.*`](https://docs.konghq.com/gateway/3.4.x/key-concepts/routes/expressions/#matching-fields) match fields are now accessible in HTTP routes defined using expressions.
* Modified the current AWS Vault backend to support `CredentialProviderChain` so that users can
choose not to use `AK-SK` environment variables to grant IAM role permissions.
* The HashiCorp Vault secrets management backend now supports the AppRole authentication method.
...
* You can now use an RBAC token to authenticate while using 
[group mapping with Kong Manager](https://docs.konghq.com/gateway/3.4.x/kong-manager/auth/oidc/mapping/) (for example, with OIDC or LDAP).
* Introduced the new endpoint [`/schemas/vaults/:name`](https://docs.konghq.com/gateway/api/admin-ee/latest/#/operations/get-schemas-vaults-vault_name) for retrieving the schema of a vault. 
...
which is called when there is a change in plugin entities. 
It receives an array of current plugin configurations, or nil if there are no active configurations.
Learn more about this function in the guide for [Implementing Custom Logic](https://docs.konghq.com/gateway/3.4.x/plugin-development/custom-logic/) for plugins.
...
* [**JWT Signer**](/plugins/jwt-signer/) (`jwt-signer`), [**LDAP Authentication Advanced**](/plugins/ldap-auth-advanced/) (`ldap-auth-advanced`),
[**OAuth 2.0 Introspection**](/plugins/oauth2-introspection/) (`oauth2-introspection`), [**OpenID Connect**](/plugins/openid-connect/) (`openid-connect`), and 
[**SAML**](/plugins/saml) (`saml`)
  * Added support for consumer group scoping by using the PDK `kong.client.authenticate` function.
...
#### Plugins
* **[AppDynamics](/plugins/app-dynamics/)**: Added `CONTROLLER_CERTIFICATE_FILE` and `CONTROLLER_CERTIFICATE_DIR` environment variable config for the AppDynamics plugin to use a self-signed certificate.
...
#### Plugins
* **[OAS-Validation](/plugins/oas-validation/)**: Fixed an issue where cookie parameters were not being validated.
...
### Breaking Changes
#### Plugins
* [**SAML**](/plugins/saml) (`saml`): Adjusted the priority of the SAML plugin to 1010 to correct the integration between the SAML plugin and other consumer-based plugins.
...
#### Plugins
* [**AWS Lambda**](/plugins/aws-lambda) (`aws-lambda`): The AWS-Lambda plugin has been refactored by using `lua-resty-aws` as an underlying AWS library. The refactor simplifies the AWS-Lambda plugin code base and adds support for multiple IAM authenticating scenarios.
...
* The default value of the [`dns_no_sync`](https://docs.konghq.com/gateway/3.4.x/reference/configuration/#dns_no_sync) option has been changed to `on`.
[#11871](https://github.com/kong/kong/pull/11871).
...
* [**SAML**](/plugins/saml/) (`saml`): Adjusted the severity of `session was not found` messages to `info`.
...
*  [**GraphQL Rate Limiting Advanced**](/plugins/graphql-rate-limiting-advanced/) (`graphql-rate-limiting-advanced`): The schema validation has been updated so that Redis cluster mode is now supported. This schema change does not impact other implementations of this plugin.
...
* [**OpenTelemetry**](/plugins/opentelemetry/) (`opentelemetry`): A new value is added to the parameter `header_type`, enabling Kong to seamlessly inject Datadog headers into forwarded requests' headers when communicating with upstream services.
...
* [**OpenTelemetry**](/plugins/opentelemetry/) (`opentelemetry`): fix an issue that resulted in invalid parent IDs in the propagated tracing headers
* [**mTLS Authentication**](/plugins/mtls-auth/) (`mtls-auth`): should not cache the network failure when performing a revocation check
* [**Canary**](/plugins/canary/) (`canary`): allow the `start` field to be a time that occurs in the past.
* [**SAML**](/plugins/saml/) (`saml`): When the Redis session storage is incorrectly configured, users now receive a 500 error instead of being redirected endlessly.
* [**OpenID Connect**](/plugins/openid-connect/) (`openid-connect`): Fix the issue on token revocation on logout where the code was revoking the refresh token when it was supposed to revoke access token when using the discovered revocation endpoint.


### Kong Manager

* Kong Manager now links directly to the [Gateway Admin API - EE (beta)](https://docs.konghq.com/gateway/api/admin-ee/3.4.0.x/)
...
images or packages, and Kong will not test package installation on Ubuntu 18.04.
    ...
    [previous versions](https://docs.konghq.com/gateway/3.1.x/install/linux/ubuntu/).
...
See the new [consumer groups](https://docs.konghq.com/gateway/api/admin-ee/3.4.0.x/#/consumer_groups/get-consumer_groups) entity.
...
* Introduced the [`cascade`](https://docs.konghq.com/gateway/latest/admin-api/workspaces/reference/#delete-a-workspace) option for 
...
free and open-source UI for Kong Gateway OSS!
  ...
  [Kong Manager OSS](https://docs.konghq.com/gateway/latest/kong-manager-oss/) allows you to view and edit all Kong Gateway objects using the Admin API. 
...
* [**IP Restriction**](/plugins/ip-restriction/) (`ip-restriction`)
...
* [**Response Transformer**](/plugins/response-transformer/) (`response-transformer`) and 
...
* [**mTLS Authentication**](/plugins/mtls-auth/) (`mtls-auth`)
...
* [**Kafka Log**](/plugins/kafka-log/) (`kafka-log`) and [**Kafka Upstream**](/plugins/kafka-upstream/) (`kafka-upstream`)
...
images or packages, and Kong will not test package installation on Ubuntu 18.04.
    ...
    patch version in the [installation instructions](https://docs.konghq.com/gateway/3.2.x/install/linux/ubuntu/).
...
#### Plugins
* [**GraphQL Rate Limiting Advanced**](/plugins/graphql-rate-limiting-advanced/) (`graphql-rate-limiting-advanced`) and [**Rate Limiting Advanced**](/plugins/rate-limiting-advanced/) (`rate-limiting-advanced`): The Redis strategy now catches strategy connection failures.
...
* [**SAML**](/plugins/saml/) (`saml`): Users will now receive a 500 error instead of being endlessly redirected when the Redis session storage is incorrectly configured.
...
* [**Response Transformer Advanced**](/plugins/response-transformer-advanced/) (`response-transformer-advanced`): The plugin no longer loads the response body when `if_status` 
doesn't match the provided status.

* [**mTLS Authentication**](/plugins/mtls-auth/) (`mtls-auth`): Fixed an issue that caused the plugin to cache network failures when running certificate revocation checks.
...
* Plugin batch queuing:
  * [**HTTP Log**](/plugins/http-log/) (`http-log`), [**StatsD**](/plugins/statsd/) (`statsd`), 
[**OpenTelemetry**](/plugins/opentelemetry/) (`opentelemetry`), and [**Datadog**](/plugins/datadog/) (`datadog`)
...
* **[Pre-function](/plugins/pre-function/) (`pre-function`) and [Post-function](/plugins/post-function/)** (`post-function`)
...
* When using the [data plane resilience feature](https://docs.konghq.com/gateway/latest/kong-enterprise/cp-outage-handling-faq/), the server-side certificate of the backend Amazon S3 or GCP Cloud Storage service will now be validated if it goes through HTTPS.
* When [managing secrets](https://docs.konghq.com/gateway/latest/kong-enterprise/secrets-management/) with an AWS or GCP backend, the backend server's certificate is now validated if it goes through HTTPS.
* Kong Enterprise now supports [using AWS IAM database authentication to connect to the Amazon RDS](https://docs.konghq.com/gateway/latest/kong-enterprise/aws-iam-auth-to-rds-database/) (PostgreSQL) database.
* Kong Manager:
...
* You can now configure [labels for data planes](https://docs.konghq.com/konnect/runtime-manager/runtime-instances/custom-dp-labels/)
...
[Kong Gateway Status API](https://docs.konghq.com/gateway/api/status/v1/).
...
* Tracing module: Renamed spans to simplify filtering on tracing backends.
  See [`kong.tracing`](https://docs.konghq.com/gateway/latest/plugin-development/pdk/kong.tracing/) for details. 
...
* [**Zipkin**](/plugins/zipkin/) (`zipkin`) and [**OpenTelemetry**](/plugins/opentelemetry/) (`opentelemetry`)
...
* [**JWE Decrypt**](/plugins/jwe-decrypt/) (`jwe-decrypt`), [**OAS Validation**](/plugins/oas-validation/) (`oas-validation`), and [**Vault Authentication**](/plugins/vault-auth/) (`vault-auth`)
...
* [**Request Termination**](/plugins/request-termination/) (`request-termination`)
...
* [**Response Transformer Advanced**](/plugins/response-transformer-advanced/) (`response-transformer-advanced`)
...
* The default PostgreSQL SSL version has been bumped to TLS 1.2. In `kong.conf`:
    ...
    * The default [`pg_ssl_version`](https://docs.konghq.com/gateway/latest/reference/configuration/#postgres-settings)
    is now `tlsv1_2`.
...
* Added the [`allow_debug_header`](https://docs.konghq.com/gateway/latest/reference/configuration/#allow_debug_header) 
...
* Sessions library upgrade [#10199](https://github.com/Kong/kong/pull/10199):
    * The [`lua-resty-session`](https://github.com/bungle/lua-resty-session) library has been upgraded to v4.0.0. This version includes a full rewrite of the session library, and is not backwards compatible.
      ...
      This library is used by the following plugins: [**Session**](/plugins/session/), [**OpenID Connect**](/plugins/openid-connect/), and [**SAML**](/plugins/saml/). This also affects any session configuration that uses the Session or OpenID Connect plugin in the background, including sessions for Kong Manager and Dev Portal.
...
* The upstream entity now has a new load balancing algorithm option: [`latency`](https://docs.konghq.com/gateway/latest/how-kong-works/load-balancing/#balancing-algorithms).
...
* The Nginx `charset` directive can now be configured with Nginx directive injections.
    Set it in Kong Gateway's configuration with [`nginx_http_charset`](https://docs.konghq.com/gateway/latest/reference/configuration/#nginx_http_charset)
...
* Added the new configuration parameter [`ssl_session_cache_size`](https://docs.konghq.com/gateway/latest/reference/configuration/#ssl_session_cache_size), 
...
* [`status_listen`](https://docs.konghq.com/gateway/latest/reference/configuration/#status_listen) now supports HTTP2. [#9919](https://github.com/Kong/kong/pull/9919)
...
* **FIPS Support**:
  * The OpenID Connect, Key Authentication - Encrypted, and JWT Signer plugins are now [FIPS 140-2 compliant](https://docs.konghq.com/gateway/latest/kong-enterprise/fips-support/). 
    ...
    If you are migrating from {{site.base_gateway}} 3.1 to 3.2 in FIPS mode and are using the `key-auth-enc` plugin, you should send [PATCH or POST requests](/plugins/key-auth-enc/#create-a-key) to all existing `key-auth-enc` credentials to re-hash them in SHA256.
...
See the documentation for more detail on [interpreting audit logs](https://docs.konghq.com/gateway/latest/kong-enterprise/audit-log/#kong-manager-authentication).
...
* [**Session**](/plugins/session/) (`session`), [**OpenID Connect**](/plugins/openid-connect/) (`openid-connect`), and [**SAML**](/plugins/saml/) (`saml`)
...
* [**GraphQL Rate Limiting Advanced**](/plugins/graphql-rate-limiting-advanced/) (`graphql-rate-limiting-advanced`) and [**Rate Limiting Advanced**](/plugins/rate-limiting-advanced/) (`rate-limiting-advanced`)
...
* [**Datadog**](/plugins/datadog/) (`datadog`),[**OpenTelemetry**](/plugins/opentelemetry/) (`opentelemetry`), and [**StatsD**](/plugins/statsd/) (`statsd`)
...
* Kong Manager with OIDC:
  * Added the configuration option
  [`admin_auto_create`](https://docs.konghq.com/gateway/latest/kong-manager/auth/oidc/mapping/) to enable or disable automatic admin creation.
...
- You can now [delete consumer group configurations](https://docs.konghq.com/gateway/latest/kong-enterprise/consumer-groups/#delete-consumer-group-configurations)
...
- **Kong Manager**:
  ...
  - You can now configure vaults for managing secrets through Kong Manager.
    ...
    See the [Vault Backends references](https://docs.konghq.com/gateway/latest/kong-enterprise/secrets-management/backends/)
...
- Added support for [dynamically changing the log level](https://docs.konghq.com/gateway/latest/production/logging/update-log-level-dynamically/).
...
see [Key management](https://docs.konghq.com/gateway/latest/reference/key-management/).
...
- Added HTTP CONNECT forward proxy support for hybrid mode connections. New configuration
  ...
  For more information, see [CP/DP Communication through a Forward Proxy](https://docs.konghq.com/gateway/latest/production/networking/cp-dp-proxy/).
...
- [**Vault Authentication**](/plugins/vault-auth/) (`vault-auth`)
...
- [**Proxy Cache Advanced**](/plugins/proxy-cache-advanced/) (`proxy-cached-advanced`)
...
Operators must upgrade to 3.0.x.x before they can perform a rolling upgrade to 3.1.0.0. For more information, see [Upgrade Kong Gateway 3.1.x](https://docs.konghq.com/gateway/3.1.x/upgrade/).
...
* [Request Transformer Advanced](/plugins/request-transformer-advanced/) (`request-transformer-advanced`)
  * Values stored in `key:value` pairs in this plugin's configuration are now referenceable, which means they can be stored as [secrets](https://docs.konghq.com/gateway/latest/kong-enterprise/secrets-management/) in a vault.
...
Review the [breaking changes and deprecations](#breaking-changes-and-deprecations) and the [known limitations](#known-limitations) before attempting to [upgrade](https://docs.konghq.com/gateway/latest/upgrade/).

### Features

#### Enterprise

* Kong Gateway now supports [dynamic plugin ordering](https://docs.konghq.com/gateway/3.0.x/kong-enterprise/plugin-ordering/).
...
* [Secrets management](https://docs.konghq.com/gateway/3.0.x/kong-enterprise/secrets-management/) is now generally available.
...
* Introduced the tracing API, which is compatible with OpenTelemetry API specs, and
  ...
  [`opentelemetry_tracing`](https://docs.konghq.com/gateway/3.0.x/reference/configuration/#opentelemetry_tracing) and [`opentelemetry_tracing_sampling_rate`](https://docs.konghq.com/gateway/3.0.x/reference/configuration/#opentelemetry_tracing_sampling_rate) options.
...
* [Kafka Upstream](/plugins/kafka-upstream/) (`kafka-upstream`) and
[Kafka Log](/plugins/kafka-log) (`kafka-log`)
...
* [ACL](/plugins/acl/) (`acl`), [Bot Detection](/plugins/bot-detection/) (`bot-detection`), and [IP Restriction](/plugins/ip-restriction/) (`ip-restriction`)
...
* [Rate Limiting](/plugins/rate-limiting/) (`rate-limiting`), [Rate Limiting Advanced](/plugins/rate-limiting-advanced/) (`rate-limiting-advanced`), and [Response Rate Limiting](/plugins/response-ratelimiting/) (`response-ratelimiting`):
...
* [Proxy Cache](/plugins/proxy-cache/) (`proxy-cache`), [Proxy Cache Advanced](/plugins/proxy-cache-advanced/) (`proxy-cache-advanced`), and [GraphQL Proxy Cache Advanced](/plugins/graphql-proxy-cache-advanced/) (`graphql-proxy-cache-advanced`)
...
during the initialization of the [keyring module](https://docs.konghq.com/gateway/latest/kong-enterprise/db-encryption/).
...
* [HMAC Authentication](/plugins/hmac-auth/) (`hmac-auth`)
  * Removed deprecated signature format using `ngx.var.uri`.
    [#8558](https://github.com/Kong/kong/pull/8558)

* [LDAP Authentication](/plugins/ldap-auth/) (`ldap-auth`)
...
* [Rate Limiting](/plugins/rate-limiting/) (`rate-limiting`) and [Response Rate Limiting](/plugins/response-ratelimiting/) (`response-ratelimiting`)
...
* Added a unique Request ID that is now populated in the error log, access log, error templates, log serializer, and a new `X-Kong-Request-Id` header. 
  This configuration can be customized for upstreams and downstreams using the 
  [`headers`](https://docs.konghq.com/gateway/2.8.x/reference/configuration/#headers) and 
  [`headers_upstream`](https://docs.konghq.com/gateway/2.8.x/reference/configuration/#headers_upstream) configuration options. 
  [#11663](https://github.com/Kong/kong/pull/11663)
...
* Added support for the debug request header `X-Kong-Request-Debug-Output`, 
  which lets you observe the time consumed by specific components in a given request.
  Enable it using the 
  [`request_debug`](https://docs.konghq.com/gateway/2.8.x/reference/configuration/#request_debug) configuration parameter.
  This header helps you diagnose the cause of any latency in Kong Gateway.
  See the [Request Debugging](https://docs.konghq.com/gateway/latest/production/debug-request/) guide for more information.
  [#11627](https://github.com/Kong/kong/pull/11627)
...
* [GraphQL Proxy Cache Advanced](/plugins/graphql-proxy-cache-advanced/) (`graphql-proxy-cache-advanced`) and [Proxy Cache Advanced](/plugins/proxy-cache-advanced/) (`proxy-cache-advanced`)
...
* When using secrets management in free mode, only the [environment variable](https://docs.konghq.com/gateway/2.8.x/plan-and-deploy/security/secrets-management/backends/env/) backend is available. AWS, GCP, and HashiCorp vault backends require an Enterprise license.
...
* [Proxy Caching Advanced](/plugins/proxy-cache-advanced/) (`proxy-cache-advanced`)
...
images or packages, and Kong will not test package installation on Amazon Linux 1.
    ...
    [previous versions](https://docs.konghq.com/gateway/2.8.x/install-and-run/amazon-linux/).
...
Debian 8 ("Jessie").
    ...
    [previous versions](https://docs.konghq.com/gateway/2.8.x/install-and-run/debian/).
...
images or packages, and Kong will not test package installation on Ubuntu 16.04.
    ...
    [previous versions](https://docs.konghq.com/gateway/2.8.x/install-and-run/ubuntu/).
...
* [Kafka Upstream](/plugins/kafka-upstream/) (`kafka-upstream`) and [Kafka Log](/plugins/kafka-log/) (`kafka-log`)
  * Added support for the `SCRAM-SHA-512` authentication mechanism.
...
* Fixed an issue with keyring encryption, where the control plane would crash if any errors occurred
during the initialization of the [keyring module](https://docs.konghq.com/gateway/latest/plan-and-deploy/security/db-encryption/).
...
* Kong Manager with OIDC:
  * Added the configuration option
  [`admin_auto_create_rbac_token_disabled`](https://docs.konghq.com/gateway/latest/configure/auth/kong-manager/oidc-mapping/)
...
[`anonymous_reports`](https://docs.konghq.com/gateway/latest/reference/configuration/#anonymous_reports).
...
[secrets management and vault support](https://docs.konghq.com/gateway/latest/kong-enterprise/secrets-management/).
...
making your environment more secure.
  ...
  *referenceable*. See the plugin section of the Kong Gateway 2.8 changelog for
  ...
  [getting started guide](https://docs.konghq.com/gateway/latest/kong-enterprise/secrets-management/getting-started/),
  and check out the documentation for the Kong Admin API [`/vaults-beta` entity](https://docs.konghq.com/gateway/latest/admin-api/#vaults-beta-entity).
...
* You can now configure [`cluster_max_payload`](https://docs.konghq.com/gateway/latest/reference/configuration/#cluster_max_payload)
...
* [OpenID Connect](/plugins/openid-connect/) (`openid-connect`)
  ...
  * **Beta feature:** The `client_id`, `client_secret`, `session_secret`, `session_redis_username`,
  ...
  [secrets](https://docs.konghq.com/gateway/latest/kong-enterprise/secrets-management/getting-started/)
  in a vault. References must follow a [specific format](https://docs.konghq.com/gateway/latest/kong-enterprise/secrets-management/reference-format/).

* [Forward Proxy Advanced](/plugins/forward-proxy/) (`forward-proxy`)
...
* [Vault Authentication](/plugins/vault-auth/) (`vault-auth`)
  ...
  * **Beta feature:** The `vaults.vault_token` form field is now marked as
  ...
  [secret](https://docs.konghq.com/gateway/latest/kong-enterprise/secrets-management/getting-started/)
  in a vault. References must follow a [specific format](https://docs.konghq.com/gateway/latest/kong-enterprise/secrets-management/reference-format/).
...
* [Proxy Cache Advanced](/plugins/proxy-cache-advanced/) (`proxy-cache-advanced`)
  ...
  * **Beta feature:**  The `redis.password`, `redis.sentinel_username`, and `redis.sentinel_password`
  ...
  securely stored as [secrets](https://docs.konghq.com/gateway/latest/kong-enterprise/secrets-management/getting-started/)
  in a vault. References must follow a [specific format](https://docs.konghq.com/gateway/latest/kong-enterprise/secrets-management/reference-format/).
...
* Fixed an issue where [`cluster_telemetry_server_name`](https://docs.konghq.com/gateway/latest/reference/configuration/#cluster_telemetry_server_name)
was not automatically generated and registered if it was not explicitly set.

* Fixed the [`cluster_allowed_common_names`](https://docs.konghq.com/gateway/latest/reference/configuration/#cluster_allowed_common_names)
...
* [OAuth2 Introspection](/plugins/oauth2-introspection/) (`oauth2-introspection`)
...
* The external `go-pluginserver` project is considered deprecated in favor of
  the [embedded server approach](https://docs.konghq.com/gateway/latest/reference/external-plugins/).
