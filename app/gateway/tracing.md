...
description: "Describes the OpenTelemetry tracing capabilities of {{site.base_gateway}}."
...
OpenTelemetry tracing replaces the deprecated Granular Tracing feature.
...
The following instrumentations can be used by any plugins built on top of Kong's [tracing API](#tracing-api), for example, 
...
The tracing API is in the `kong.tracing` namespace. The tracing API follows the [OpenTelemetry API specification](https://github.com/open-telemetry/opentelemetry-specification/blob/main/specification/trace/api.md).
