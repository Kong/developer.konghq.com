...
generate new certificates and replace them on the existing nodes. For more information, see [Renew Data Plane certificates in {{site.konnect_short_name}}](/how-to/renew-data-plane-certs/).
...
that references the URI of OCSP responder that can be reached from the Control Plane. [`cluster_ocsp`](/gateway/configuration/#cluster-ocsp) affects all hybrid mode connections established from a Data Plane to its Control Plane.
