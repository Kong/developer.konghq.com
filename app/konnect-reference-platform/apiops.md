...
delivery of APIs to production in auditable, consistent, and repeatable manner.
...
The GitHub Actions workflow files are managed by the [{{site.konnect_short_name}} Orchestrator](/konnect-reference-platform/orchestrator) and deployed 
to the platform repository. When the orchestrator's reconciliation loop runs, it will check for changes to the workflow files and stage them as a PR for 
platform administrators to review and approve. Once the PR is approved, the workflow files are deployed and begin to operate on API specifications
in the platform repository. The process of initializing the platform repository is covered in the 
[{{site.konnect_short_name}} Reference Platform how-to guide](/konnect-reference-platform/how-to/).
...
* **OpenAPI to Kong**: This workflow takes an OpenAPI specification and converts it to 
  a decK file that can be used by {{site.base_gateway}}. Additionally, service specific patches are
  applied to the deck file allowing administrators to set overrides for specific APIs.
* **Stage decK changes**: This workflow takes the decK file generated by the OpenAPI to Kong workflow
  and compares it to the current state of the {{site.base_gateway}}. It generates a diff and stages the changes
  as a PR. 
* **decK sync**: This workflow is triggered by the merging of deck configuration files to the main branch (by the previous workflow PR approval).
...
following governance controls and best practices. This is done using the `deck file lint` command 
which is executed in a GitHub Action workflow. These conformance workflows are run on PRs that contain
changes to API specifications and deck configuration files. 
...
{% mermaid %}
flowchart TB
a@{ shape: lin-cyl, label: "Source Code Repository"}
b@{ shape: rect, label: "Process"}
c@{ shape: lin-doc, label: "Pull Request"}
e@{ shape: doc, label: "File"}
f@{ shape: docs, label: "Files"}
{% endmermaid %}
