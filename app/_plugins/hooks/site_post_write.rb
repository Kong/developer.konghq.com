...
class LlmsTxtWriter # rubocop:disable Style/Documentation
  ...
  def docs
    @docs ||= begin
      ...
      slugs = grouped.keys.compact.sort_by do |slug|
        resolve_name(slug)
      end
      slugs << nil if grouped.key?(nil)

      slugs.map do |slug|
        {
          'name' => slug ? resolve_name(slug) : 'Other',
          'pages' => grouped[slug].sort_by(&:url)
        }
      end
    end
  end
  ...
  def api_pages
    @api_pages ||= pages.select { |p| p.data['content_type'] == 'api' || p.data['layout'] == 'api/errors' }
  end
  ...
  def resolve_name(slug)
    @site.data.dig('products', slug, 'name') || @site.data.dig('tools', slug, 'name') || slug
  end
