...
The client credentials flow is a [standard OAuth flow](https://datatracker.ietf.org/doc/html/rfc6749#section-4.4) for using pre-determined credentials (such as a `client_id` and secret) for authenticating a client to Kong Identity. This is typically used in situations where the authentication context is _only the client itself_, and for this reason is often used for Machine-to-Machine (M2M) use cases, such as a microservice authenticating to a Gateway. 

The following diagram shows how the client credentials flow works with Kong Identity:
...
The specific client credentials that Kong Identity supports are the `client_id` (which is determined when the client is created on the Auth Server) and `client_secret`. 
If you create the client by sending a [POST request to the `/auth-servers/{authServerId}/clients` endpoint](/api/konnect/kong-identity/v1/#/operations/createAuthServerClient), then the client ID and initial secret will be generated automatically. 
If you send a [PUT request to the `/auth-servers/{authServerId}/clients/{clientId}` endpoint](/api/konnect/kong-identity/v1/#/operations/replaceAuthServerClient), then you can set the `client_id` and/or `client_secret` explicitly.

{:.info}
> **Note:** For security reasons, {{site.konnect_short_name}} does not store the actual value of the secrets associated with a client. As such, the actual value will only ever be presented once, at the time the secret is generated. This might be when a new client is created with an autogenerated secret, or when additional client secrets are generated for rotation.
