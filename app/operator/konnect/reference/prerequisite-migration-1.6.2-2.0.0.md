...
1. **Locate Your KonnectExtension Resource Using `konnectID`**
    ...
    Find your `KonnectExtension` manifest that uses the `konnectID` field. It will look similar to the following (with a placeholder for the ID)
...
2. **Create a KonnectGatewayControlPlane in Mirror Mode**
    ...
    Next, create a [`KonnectGatewayControlPlane` resource](../dataplanes/konnectextension.md#konnectgatewaycontrolplane-spec) in your cluster that mirrors the same Konnect control plane you previously referenced with `konnectID`. This enables the new reference type required by KO 2.0.
