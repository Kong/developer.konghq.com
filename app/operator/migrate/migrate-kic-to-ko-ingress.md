---
title: "Migrate an Ingress configuration to {{ site.operator_product_name }}"
description: "Migrate Ingress routing from KIC to Kong Operator while preserving your existing Service and client address."
...
Migrate your existing `Ingress` resources to {{ site.operator_product_name }} while retaining the Service and address your clients use. Both controllers will read the same Ingress resources during migration; you do not need to convert them to Gateway API routes.
...
The new resources are named `kong-migrated`. If that name is already in use, choose another name throughout this guide. Set `INGRESS_CLASS` to the class your existing Ingress resources use.
...
The operator uses a `Gateway` to manage the deployment even though application routing still uses Ingress resources. Set `controlPlaneOptions.ingressClass` to your existing ingress class so the replacement processes those resources.
...
Both installations now translate the same Ingress resources for their own data planes. No route attachment step is needed.
...
Repeat your application checks through the original client address. Leave the old pods running so existing connections can drain. If you use an AWS NLB with IP targets, wait for the new targets to become healthy and old targets to drain before cleanup; see [AWS load balancer cutover](#aws-load-balancer-cutover).
...
1. Uninstall the Helm release:
   ...
   Helm should report the proxy Service as kept.

1. Verify traffic is still healthy on your address.

The uninstall also removes the old `IngressClass`. See [IngressClass after uninstall](#ingressclass-after-uninstall) if other tooling depends on it.

The migration is complete. Keep the retained Service in your managed manifests. {{ site.operator_product_name_short }} publishes the address of its own internal Service in resource status; see [Published addresses](#published-addresses) if your tooling uses that field.

## Troubleshooting

| Symptom | Cause | Resolution |
|---|---|---|
| The new Gateway is programmed but serves `404` for every Ingress resource | `controlPlaneOptions.ingressClass` is not set, or does not match the class your Ingress resources reference | Set `ingressClass` in the `GatewayConfiguration`. Without it, Ingress resources are not processed at all. |
...
| Ingress `status.loadBalancer` shows a cluster IP after the migration | {{ site.operator_product_name_short }} publishes the address of its own data plane ingress Service, which is `ClusterIP` on the recommended path | Expected. See [Optional: Transfer address management](#optional-transfer-address-management). |

## Additional details

### Published addresses

The operator publishes its own data plane Service's `ClusterIP`, which differs from the address on your retained Service. Account for this in tooling that reads resource status.

While both controllers run, the published Ingress `status.loadBalancer` address can alternate between them. Validate application traffic through the retained Service.

### GatewayConfiguration settings

The example sets `cache.initSyncDuration` to `30s` to give the control plane more time to initialize its Kubernetes caches before the first configuration push. Its default is `5s`. See the [`GatewayConfiguration` reference](/operator/reference/custom-resources/#gatewayconfiguration) for other settings.

### AWS load balancer cutover

On AWS with `aws-load-balancer-nlb-target-type: ip`, the load balancer registers pod IPs directly rather than node ports, so the selector change is a target group membership change. New targets must pass health checks while old targets drain, which makes the cutover gradual rather than instant. This is not an outage, because the old pods keep serving while they are removed from the target group, but wait for the target group to converge fully before continuing to cleanup. With the default `instance` target type, the load balancer is unaffected by the change.

### Stop the old controller before uninstall

If you want to stop {{ site.kic_product_name_short }} from reconciling before the uninstall, you can scale its controller down first. This is only possible with the `ingress` chart, which runs the controller in its own Deployment: `kubectl scale deploy -n ${NAMESPACE} ${RELEASE_NAME}-controller --replicas=0`. The `kong` chart runs the controller as a container in the same Deployment as the gateway, so there is nothing to scale independently. Uninstall the release instead.

### IngressClass after uninstall

The uninstall also removes the `IngressClass` object. This does not affect routing: Ingress resources are matched on the `spec.ingressClassName` string, and the `IngressClass` object is only read to resolve the default class annotation. Recreate it only if other tooling depends on it.
