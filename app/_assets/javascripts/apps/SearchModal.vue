<template>
  <button aria-label="Search" class="cursor-pointer flex gap-4 items-center justify-between w-fit border rounded-md py-[7px] px-3 text-secondary bg-secondary border-b border-primary leading-4" @click="openModal">
    <span class="flex items-center gap-2 flex-shrink-0">
      ...
      <span class="hidden xl:group-[:not(.ai-assistant-open)]/body-tag:flex 3xl:flex text-sm">Search</span>
    </span>
    ...
    <div class="modal-content-wrapper">
      <div class="modal-content" aria-modal="true" role="dialog" @click.stop>
        <div class="modal-content-inner" v-bind="autocomplete.getRootProps({})">
          <div class="flex flex-col p-4 border-b border-primary">
            <form class="w-full gap-2 flex items-center" v-bind="formProps" @submit="() => {this.inputElement.focus()}">
              ...
              <div class="flex items-center border border-primary rounded-3xl h-full gap-1 pl-2 w-fit" v-for="tag in state.context.tagsPlugin.tags">
          ...
          <div class="md:flex w-full p-4 items-center flex-grow-0 flex-shrink-0 border-t border-primary justify-between">
