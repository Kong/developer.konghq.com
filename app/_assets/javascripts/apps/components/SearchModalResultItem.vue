<template>
  <div class="flex w-full">
    <a :href="relativeUrl" class="flex gap-3 w-full p-3 rounded-md bg-primary border border-primary/5 shadow-primary hover:no-underline hover:border-brand-saturated/40 hover:shadow-hover-card  focus:border-brand-saturated/40 focus:shadow-hover-card" tabindex="0">
      <div class="flex p-2 rounded bg-secondary items-center h-fit">
        <svg v-if="item.content_type === 'how_to'" width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <svg v-else-if="item.content_type === 'plugin' || item.content_type === 'plugin_example'" width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <div class="flex flex-col w-full gap-1">
        ...
        <div class="flex gap-3 pt-1 items-center">
          <div v-for="product in products" class="flex gap-1.5 items-center">
              <ProductIcon :name="product.value" :background="false" />
              <span class="text-secondary text-xs">{{ product.label }}</span>
          </div>
        </div>
      </div>
    </a>
...
const availableProducts = window.searchFilters.products;
...
export default {
  ...
  computed: {
    ...
    products() {
      return availableProducts.filter((p) => this.item.products.includes(p.value));
    }
  },
};
</script>