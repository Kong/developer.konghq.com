...
{:.warning}
> The `from` array is removed. Rewrite `from` entries as `rules`. A `from` entry targeting
> `kind: Mesh`, meaning all clients, becomes a single catch-all rule:
>
> ```yaml
> # before
> spec:
...
> `kuma.io/protocol` tag is served as plain TCP after upgrading, and loses the HTTP timeouts
> in this policy along with the other L7 filters. Set `networking.inbound[].protocol`
> explicitly. Nothing rejects the resource, so the change is silent.
