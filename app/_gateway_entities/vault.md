...
> * <sup>2</sup>: For the same reason that secrets are resolved at the Konnect level, it is currently not possible to use Konnect Config Store secrets directly in Lua code via the Kong PDK, for example.
> * <sup>3</sup>: In Konnect, the {{site.base_gateway}} license is managed and stored by {{site.konnect_short_name}}, and doesn't need to be stored manually in any Vault.
