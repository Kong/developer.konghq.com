...
By default, the credentials generated by the OAuth2 plugin are scoped to a single Service. Setting `config.global_credentials` to `true` allows the plugin to use the same credentials with any other Service whose OAuth2 plugin configuration also sets `global_credentials` to `true`.

Credential sharing applies to access tokens only. Authorization codes aren't shared: an authorization code created by one plugin instance can't be exchanged for an access token by a different plugin instance, even when both plugin instances set `global_credentials` to `true`.

This behavior was introduced in {{site.base_gateway}} 3.3.0.0. For more details, see the [plugin changelog](/plugins/oauth2/changelog/).
