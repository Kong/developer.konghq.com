...
## Global credentials {% new_in 3.3 %}

By default, the credentials generated by the OAuth2 plugin are scoped to a single Service. Setting [`config.global_credentials`](/plugins/oauth2/reference/#schema--config-global-credentials) to `true` allows the plugin to use the same credentials with any other Service whose OAuth2 plugin configuration also sets `global_credentials` to `true`.
