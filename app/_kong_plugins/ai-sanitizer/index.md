...
Kong provides several AI PII Anonymizer service Docker images in a [private repository](https://hub.docker.com/repository/docker/kong/ai-pii-service/general). Each image includes a built-in NLP model and is tagged using the `version-lang_code` format. For example:
