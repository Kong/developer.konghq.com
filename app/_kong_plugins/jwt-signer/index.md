...
instead of allowing {{site.base_gateway}} to autogenerate them, we can add another parameter
...
`config.channel_token_jwks_uri` to {{site.base_gateway}} database for quicker access to them. The plugin
...
upstream services for them to verify {{site.base_gateway}}-issued tokens. The response is a standard
...
You can also make a loopback to this endpoint by routing {{site.base_gateway}} proxy to this URL.
...
Sometime you might want to rotate the keys {{site.base_gateway}} uses for signing tokens specified with
...
`config.channel_token_jwks_uri`. {{site.base_gateway}} stores and uses at most two set of keys:
