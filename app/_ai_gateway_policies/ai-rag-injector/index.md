...
However, this won't work in hybrid mode or {{site.konnect_short_name}} because the control plane can't access the policy's backend storage.
...
local function get_policy_by_id(id)
...
local policy_id, content = args[2], args[3]

local policy, err = get_policy_by_id(policy_id)
if err then
  ngx.log(ngx.ERR, "Failed to get policy: " .. err)
  return
end

if not policy then
  ngx.log(ngx.ERR, "Policy not found")
  return
end

local _, err = ingest_chunk(policy.config, content)
if err then
