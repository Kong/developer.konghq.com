...
rows:
  ...
  - v1: "Set the `config.model.options[].input_cost` and `config.model.options[].output_cost` on [AI Proxy Advanced](/plugins/ai-proxy-advanced/)"
    v2: "Set [Model cost parameters](/ai-gateway/model-cost-management/#model-cost-configuration) on an [AI Model](/ai-gateway/entities/ai-model/)"
