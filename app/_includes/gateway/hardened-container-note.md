...
>
> {{hardened_product_name}} needs a writable volume mounted at the `prefix` (`KONG_PREFIX`) directory, even in a read-only root filesystem. This directory holds the PID file, Unix sockets, the LMDB cache, and the generated NGINX configuration. You can redirect logs, such as `proxy_error_log`, to a separate writable path or to stderr.
>
