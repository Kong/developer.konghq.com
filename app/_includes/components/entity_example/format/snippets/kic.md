...
kubectl annotate -n kong {% if key contains "Service" %}{{ key | downcase }}{% else %}{% if key contains "Route" %}{{ key | downcase }}{% endif %}{% if key contains "Consumer" %}kong{{ key | downcase }}{% endif %}{% else %}Kong{{ key }}{% endif %} {{ target_name }} konghq.com/plugins={{ include.presenter.other_plugins }}{{ include.presenter.full_resource.metadata.name }}{% if include.presenter.other_plugins %} --overwrite{% endif %}
{% endfor -%}
```
{% else %}
{% navtabs api %}
