<!---shared with logging Policies: file-log, http-log, loggly, syslog, tcp-log, udp-log DOCS-1617 --->
...
rows:
  ...
  - log: "`latencies.kong`"
    description: |
      The internal {{site.ai_gateway}} latency, in milliseconds, that it takes to process the request.
      ...
      * For requests that generate a response within {{ site.ai_gateway }} (typically the result of an error or a policy-generated response), it is equivalent to the `X-Kong-Response-Latency` [response header](/gateway/configuration/#headers).
  ...
  - log: "`latencies.proxy`"
    description: |
      ...
      In other words, it's the time elapsed between transferring the request to the final Service and when {{site.ai_gateway}} starts receiving the response.
