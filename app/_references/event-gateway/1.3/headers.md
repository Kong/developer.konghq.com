...
title: "Internal {{site.event_gateway}} headers"
...
This document lists every Kafka record header KEG may add or interpret.

## enc

### `kong/enc`

Encryption metadata: which of record key/value is encrypted and the key id(s) used. Also carries the (deduplicated) list of key ids referenced by per-field encrypted payloads when the `encrypt_fields` sub-policy is configured (see MADR 046). The `decrypt` and `decrypt_fields` policies read this to know which key(s) to fetch.

|Field        |Value     |
|:------------|:---------|
|Encoding     |`binary` (schema: `EncryptionMetadata`)|
|Max size     |_unbounded_|
|Producer     |encrypt policy / encrypt_fields sub-policy|
|Consumer     |decrypt policy / decrypt_fields sub-policy|
|Direction    |`both`|
|Visibility   |`internal`|
