...
In a [multi-zone deployment](/mesh/v2/mesh-multizone-service-deployment/), you can use the `ZoneIngress` proxy to manage cross-zone communication.
...
{:.info}
> Since the `ZoneIngress` proxy uses [Server Name Indication (SNI)](https://en.wikipedia.org/wiki/Server_Name_Indication) to route traffic, [mTLS](/mesh/v2/policies/mutual-tls/) is required to handle cross-zone communication.
