...
{:.info}
> To use a `MeshExternalService`, you must enable a [zone egress](/mesh/v2/zone-egress/) and [mutual TLS](/mesh/v2/policies/mutual-tls/).
...
The only `type` supported is `HostnameGenerator` (this field is optional). This means that {{site.mesh_product_name}} will match traffic directed to a hostname created by the [`HostnameGenerator`](/mesh/v2/hostnamegenerator/) resource.
...
{{site.mesh_product_name}} [automatically creates a `HostnameGenerator`](/mesh/v2/hostnamegenerator/#local-meshexternalservice) resource with a `meshExternalService` selector for each `MeshExternalService` resource created.
...
`MeshExternalService` works in Universal mode without transparent proxy, but you need to manually define an outbound that targets the correct `MeshExternalService` in the [`Dataplane` configuration](/mesh/v2/data-plane-proxy/#dataplane-entity):
...
These examples use a [single-zone deployment](/mesh/v2/single-zone/) and the following `HostnameGenerator`:
...
In a [multi-zone deployment](/mesh/v2/mesh-multizone-service-deployment/), when applying resources on the global control plane, you need to:
