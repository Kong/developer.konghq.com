...
The `Dataplane` resource configures a [data plane proxy](/mesh/v2/data-plane-proxy/) (also called a sidecar). The proxy runs alongside each workload and handles all inbound and outbound traffic for that workload.
...
Use [transparent proxying](/mesh/v2/transparent-proxying/) so the workload reaches mesh services by their service name without declaring outbounds:
...
Configure [health probes](/mesh/v2/dataplane-health/) so {{site.mesh_product_name}} can detect when the workload becomes unhealthy and stop routing traffic to it:
...
Run a [built-in gateway](/mesh/v2/built-in-gateway/) for ingress traffic, configured through `MeshGateway` resources:
