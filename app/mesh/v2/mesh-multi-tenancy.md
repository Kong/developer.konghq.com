...
>
> To prevent this configuration issue proactively, you can enable the runtime flag [`runtime.kubernetes.disallowMultipleMeshesPerNamespace`](/mesh/v2/reference/kuma-cp/). When enabled, the admission webhook rejects Pod creation and Pod updates if the namespace already contains data planes in a different mesh.
>
...
You can control which data plane proxies are allowed to join the mesh using [mesh constraints](/mesh/v2/configure-data-plane-proxy-membership/).

### Policies

When creating new [policies](/mesh/v2/policies-introduction/), you must also specify which mesh they belong to:
