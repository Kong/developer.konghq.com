...
products:
  - mesh
permalink: /mesh/v2/policies/mutual-tls/
breadcrumbs:
  - /mesh/v2/
...
* [`builtin`](#using-a-builtin-ca): {{site.mesh_product_name}} automatically generates a CA root certificate and key, which are stored as a [Secret](/mesh/v2/manage-secrets/).
...
Apply the configuration with `kumactl apply -f [..]` or via the [HTTP API](/mesh/v2/interact-with-control-plane/).
{% endnavtab %}
{% endnavtabs %}
...
When using a `builtin` backend {{site.mesh_product_name}} automatically generates a root CA certificate and key that are stored as a {{site.mesh_product_name}} [Secret resource](/mesh/v2/manage-secrets/) with the following name:
...
- Universal stores secrets in the underlying [store](/mesh/v2/control-plane-configuration/#store) configured in `kuma-cp`.
...
Unlike the `builtin` backend, with `provided` you first upload the certificate and key as [Secret resource](/mesh/v2/manage-secrets/), and then reference the Secrets in the mTLS configuration.
...
The command generates a certificate at `crt.pem` and the key at `key.pem`. Generate the {{site.mesh_product_name}} Secret resources using the [Secret resource](/mesh/v2/manage-secrets/).
...
Use the `/meshes/{mesh}/dataplanes+insights/{dataplane}` endpoint of the [{{site.mesh_product_name}} HTTP API](/mesh/v2/interact-with-control-plane/) and inspect the `dataplaneInsight` object.
