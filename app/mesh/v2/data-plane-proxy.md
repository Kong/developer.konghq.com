...
* `kuma.io/zone` identifies the zone name in a [multi-zone deployment](/mesh/v2/mesh-multizone-service-deployment/). This tag is automatically created and cannot be overwritten.
...
Outbounds are not required when using [transparent proxying](/mesh/v2/transparent-proxying/). 
...
* On [Kubernetes](/mesh/v2/data-plane-kubernetes/), the control plane automatically generates the `Dataplane` entity. 
* On [Universal](/mesh/v2/data-plane-universal/), the user defines the `Dataplane` entity. 
...
* `9000`: The HTTP server that provides the [virtual probes](/mesh/v2/annotations/#kuma-io-virtual-probes) functionalities (in version 2.8 and earlier). It is automatically enabled on Kubernetes. On Universal, it needs to be enabled explicitly.
...
* {% new_in 2.9 %} `9001`: The HTTP server that provides the [application probe proxy](/mesh/v2/annotations/#kuma-io-application-probe-proxy-port) functionalities. It can be customized using these methods:
