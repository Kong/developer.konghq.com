...
[Policies](/mesh/v2/policies-introduction/) use secrets at runtime.
...
Global secrets are used for internal purposes, to store [zone tokens](/mesh/v2/zone-proxy-auth/#zone-token) or [user token](/mesh/v2/authentication-with-the-api-server/#user-token) signing keys for example.
...
Here is an example of how to use a {{site.mesh_product_name}} `Secret` with a `provided` [Mutual TLS](/mesh/v2/policies/mutual-tls/#using-a-provided-ca) backend.
