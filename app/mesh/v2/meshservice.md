...
The [`HostnameGenerator` resource](/mesh/v2/hostnamegenerator/) is used to manage hostnames for `MeshService` resources. The `VirtualOutbound` policy isn't supported by the `MeshService` resource.
...
* Moving to a [`MeshMultiZoneService`](/mesh/v2/meshmultizoneservice/) to keep the same behavior.
