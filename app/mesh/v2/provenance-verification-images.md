...
related_resources:
  ...
  - text: "FIPS Support"
    url: /mesh/v2/enterprise/#fips-140-2-support
  - text: Verify build provenance for {{site.mesh_product_name}} binaries
    url: /mesh/v2/provenance-verification-binaries/
...
2. Parse the image manifest using `regctl`:
   ...
   ```sh
   export IMAGE_DIGEST=$(regctl manifest digest kong/kuma-cp:{{page.release.version}})
   ```
