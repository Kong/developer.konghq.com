...
related_resources:
  - text: Version-specific upgrade notes
    url: /mesh/v2/version-specific-upgrade-notes/
...
{:.warning}
> Some versions have specific upgrade instructions. Make sure to read the [version specific upgrade notes](/mesh/v2/version-specific-upgrade-notes/) for more information.
...
Although control planes within a zone don't connect to each other, they share a common [store](/mesh/v2/control-plane-configuration/#store) (usually Kubernetes or PostgreSQL). Compatibility of the storage layer also follows the minor version rules:
...
{:warning}
> Some feature flags may not provide backward compatibility. When this is the case, it's clearly documented in the [control plane configuration](/mesh/v2/reference/kuma-cp/) and will be part of the `experimental` section.
>
