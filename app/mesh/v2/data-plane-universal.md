...
related_resources:
  ...
  - text: 'Data plane on Kubernetes'
    url: '/mesh/v2/data-plane-kubernetes/'
  ...
  - text: Deploy a multi-zone global control plane in Universal mode with Docker
    url: /mesh/v2/deploy-multi-zone-global-control-plane-universal-docker/
  - text: Zone ingress
...
> To learn how to generate tokens, see the [data plane authentication docs](/mesh/v2/data-plane-proxy-authentication/#data-plane-proxy-token).

When transparent proxying isn't enabled, the outbound service dependencies have to be manually specified in the [`Dataplane`](/mesh/v2/data-plane-proxy/#dataplane-entity) entity.
...
Consider using [service probes](/mesh/v2/dataplane-health/#kubernetes-and-universal-service-probes) to mark the data plane proxy as healthy only after all health checks are passed.
...
Consider using [service probes](/mesh/v2/dataplane-health/#kubernetes-and-universal-service-probes) to mark data plane proxy as unhealthy when it is in draining state.
...
The `Dataplane` resource is created using the HTTP API or [kumactl](/mesh/v2/cli/#kumactl). 
...
To allow the data plane to join the mesh in a graceful way, you can use [service probes](/mesh/v2/dataplane-health/#kubernetes-and-universal-service-probes).
...
To allow the data plane to leave the mesh in a graceful way, you can use [service probes](/mesh/v2/dataplane-health/#kubernetes-and-universal-service-probes).
...
If the `admin` section is empty or if the port is equal to zero, then the default value for port will be taken from the [{{site.mesh_product_name}} control plane configuration](/mesh/v2/reference/kuma-cp/).
