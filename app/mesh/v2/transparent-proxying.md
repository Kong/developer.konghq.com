...
{{site.mesh_product_name}} uses [`iptables`](https://linux.die.net/man/8/iptables) and offers additional, experimental support for [`eBPF`](/mesh/v2/cni/#merbridge-cni-with-ebpf).
...
{{site.mesh_product_name}} integrates with the service naming provided by Kubernetes DNS, and also provides its own [{{site.mesh_product_name}} DNS](/mesh/v2/dns/) for multi-zone service naming.

## Universal

On Universal, `kuma-dp` leverages the [data plane proxy specification](/mesh/v2/data-plane-universal/#dataplane-resource-configuration) for receiving incoming requests on a pre-defined port.
...
* Universal service naming with the `.mesh` [DNS domain](/mesh/v2/dns/) instead of explicit outbounds like `https://localhost:10001`.
...
{{site.mesh_product_name}} comes with the [`kumactl` executable](/mesh/v2/cli/#kumactl), which can help you prepare the host for transparent proxying. 
...
Once this is configured, you'll be able to reach the Service on the same IP and port as before installing transparent proxy, but the traffic will go through Envoy. You can also connect to Services using [{{site.mesh_product_name}} DNS](/mesh/v2/dns/).
...
You can exclude ports on a whole {{site.mesh_product_name}} deployment with the following {{site.mesh_product_name}} [control plane configuration](/mesh/v2/control-plane-configuration/):
```sh
...
The reachable backends feature provides similar functionality to [reachable services](#reachable-services), but it applies to the [`MeshService`](/mesh/v2/meshservice/), [`MeshExternalService`](/mesh/v2/meshexternalservice/), and [`MeshMultiZoneService`](/mesh/v2/meshmultizoneservice/) resources.
...
- `refs`: A list of all resources your application should track and communicate with.
  - `kind`: The type of resource. Possible values include:
    - [`MeshService`](/mesh/v2/meshservice/)
    - [`MeshExternalService`](/mesh/v2/meshexternalservice/)
    - [`MeshMultiZoneService`](/mesh/v2/meshmultizoneservice/)
