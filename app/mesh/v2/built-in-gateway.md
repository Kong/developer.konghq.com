...
related_resources:
  ...
  - text: Delegated gateways
    url: /mesh/v2/delegated-gateways/
  - text: Kubernetes Gateway API
    url: /mesh/v2/kubernetes-gateway-api/
major_version:
  mesh: 2

---

In {{site.mesh_product_name}}, gateways allow you to manage [ingress traffic](/mesh/v2/ingress/) between a client and the Services in your meshes. You can either use a [delegated gateway](/mesh/v2/delegated-gateways/), such as {{site.base_gateway}}, or a built-in gateway. 
...
The process for deploying built-in gateways is different depending on whether you're running in [Kubernetes](/mesh/v2/kubernetes/) or [Universal](/mesh/v2/get-started/universal/install/) mode.

{% navtabs "Environment" %}
{% navtab "Kubernetes" %}

To manage gateway instances on Kubernetes, {{site.mesh_product_name}} provides a [`MeshGatewayInstance`](/mesh/v2/meshgatewayinstance/) CRD.
...
See [the `MeshGatewayInstance` docs](/mesh/v2/meshgatewayinstance/) for more information.
{% endnavtab %}
{% navtab "Universal" %}
...
Then, run `kuma-dp` with the `Dataplane` configuration file and a [token](/mesh/v2/data-plane-proxy-authentication/#data-plane-proxy-token):
...
For more information, see [Multi-zone deployment](/mesh/v2/mesh-multizone-service-deployment/).