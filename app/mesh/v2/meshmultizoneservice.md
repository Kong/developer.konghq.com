...
[HostnameGenerator](/mesh/v2/hostnamegenerator/) can be used to assign hostnames.

{{site.mesh_product_name}} assigns each MeshMultiZoneService a unique Virtual IP.
The default CIDR is `243.0.0.0/8` and can be changed by [configuring a zone control plane](/mesh/v2/control-plane-configuration/) with the `KUMA_IPAM_MESH_MULTI_ZONE_SERVICE_CIDR` environment variable.
