...
related_resources:
  ...
  - text: Transparent proxying
    url: '/mesh/v2/transparent-proxying/'
  - text: Kubernetes annotations and labels
    url: /mesh/v2/annotations/
  - text: Concepts
    url: /mesh/v2/concepts/
...
{{site.mesh_product_name}} DNS is only relevant when you use [transparent proxying](/mesh/v2/transparent-proxying/).
...
For [`MeshService`](/mesh/v2/meshservice/), [`MeshExternalService`](/mesh/v2/meshexternalservice/), and [`MeshMultiZoneService`](/mesh/v2/meshmultizoneservice/) resources, hostnames are produced by a [`HostnameGenerator`](/mesh/v2/hostnamegenerator/) and use the `.mesh.local` domain. {{site.mesh_product_name}} ships with default generators, so the typical hostnames you'll see are:
...
See [`HostnameGenerator`](/mesh/v2/hostnamegenerator/) to customize these templates.
...
{{site.mesh_product_name}} DNS requires [transparent proxying](/mesh/v2/transparent-proxying/) to be enabled:

- **On Kubernetes**, transparent proxying and DNS are enabled automatically whenever the `kuma-dp` sidecar proxy is injected.
- **On Universal**, follow the instructions in [transparent proxying](/mesh/v2/transparent-proxying/) to set it up first.

{:.info}
> **Note:** {{site.mesh_product_name}} DNS uses advanced networking techniques. In mixed IPv4 and IPv6 environments, we recommend specifying an [IPv6 virtual IP CIDR](/mesh/v2/ipv6-support/) so DNS responses work consistently across both stacks.

## Configuration

You can [configure](/mesh/v2/reference/kuma-cp/) {{site.mesh_product_name}} DNS in `kuma-cp`:
...
To override the configuration, [configure](/mesh/v2/reference/kuma-cp/) the bootstrap server in `kuma-cp`:
...
To override DNS configuration from the control plane, you can [configure](/mesh/v2/reference/kuma-cp/) the bootstrap server in `kuma-cp`:
