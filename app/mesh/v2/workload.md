...
related_resources:
  ...
  - text: Data plane proxy authentication
    url: /mesh/v2/data-plane-proxy-authentication/
major_version:
  mesh: 2

---

The `Workload` resource represents a logical grouping of [data plane proxies](/mesh/v2/data-plane-proxy/) that share the same workload identifier. {{site.mesh_product_name}} creates and manages `Workload` resources automatically when data plane proxies carry a `kuma.io/workload` label. On Kubernetes, set the label with a `kuma.io/workload` annotation on the Pod. On Universal, set the label directly on the `Dataplane` resource.
...
>
>   For details on preventing this configuration issue, see the [namespace-mesh constraint documentation](/mesh/v2/mesh-multi-tenancy/).
...
* Runtime enforcement: To prevent multi-mesh namespaces proactively, enable the [`runtime.kubernetes.disallowMultipleMeshesPerNamespace`](/mesh/v2/reference/kuma-cp/) flag. With the flag enabled, the admission webhook rejects Pod creation and Pod updates when the namespace already contains `Dataplane` resources in a different mesh.
