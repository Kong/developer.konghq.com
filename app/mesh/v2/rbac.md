...
faqs:
  - q: What should I do if I've locked myself out?
    a: |
        ...
        1. [Configure the control-plane](/mesh/v2/control-plane-configuration/) by setting the: `KUMA_ACCESS_TYPE` environment variable to `static`, and then restart the control-plane.
        2. Create the default `AccessRoleBinding` and `AccessRole` (as described in the [default section](/mesh/v2/rbac/)), or add new groups if necessary.
...
{:.warning}
> **Important:** [By default](/mesh/v2/rbac/), {{site.mesh_product_name}} assigns the `admin` role to everyone in the `mesh-system:authenticated` and `mesh-system:unauthenticated` groups. This means every user automatically gets admin rights, even if you create a custom binding for them, they will also inherit the default admin role.
