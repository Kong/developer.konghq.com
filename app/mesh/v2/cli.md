...
* Retrieve the state of {{site.mesh_product_name}} and the configured [policies](/mesh/v2/policies-introduction/) in every environment.
...
* `kumactl generate dataplane-token`: Generates a [data plane token](/mesh/v2/data-plane-proxy-authentication/#data-plane-proxy-token).
