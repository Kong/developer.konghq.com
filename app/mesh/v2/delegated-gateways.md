...
In delegated gateway mode, {{site.mesh_product_name}} configures an Envoy [sidecar](/mesh/v2/data-plane-proxy/) for your API gateway.
...
See [Use Kong as a delegated gateway with {{site.mesh_product_name}}](/mesh/v2/use-kong-as-delegated-gateway/) to get started with delegated gateways.
...
In order to send multi-zone traffic, you can either use the [`.mesh` address](/mesh/v2/dns/) or create a `Service` of type `ExternalName` that points to that URL.
