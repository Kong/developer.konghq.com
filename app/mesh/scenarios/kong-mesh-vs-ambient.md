...
breadcrumbs:
  - /mesh/
  - /mesh/scenarios/
description: A technical analysis of the trade-offs between sidecar-based {{site.mesh_product_name}} and sidecar-less Istio Ambient, focusing on security, latency, and operational simplicity.
products:
  - mesh
next_steps:
...
## 2. The "L7 Tax": Way point vs. Local Sidecar

Ambient claims to be faster by removing sidecars, but this only applies to simple L4 traffic. When you need **L7 features** (Retries, URL Routing, AuthZ, Observability), Ambient introduces a "Way point" proxy.

| Feature | {{site.mesh_product_name}} (Sidecar) | Istio Ambient (Way point) |
| :--- | :--- | :--- |
...
2.  **Way point**: Shared L7 proxy (Envoy-based).
...
*   **Istio Ambient**: Does not natively support VMs in the side-carless model. You end up with a "Hybrid" mess: Ambient for K8s and traditional sidecars for VMs, doubling your operational complexity.
...
| **Troubleshooting** | Predictable (Pod logs) | Complex (Ztunnel +  Way point) |
