...
breadcrumbs:
  - /mesh/
  - /mesh/scenarios/
description: Implement a zone-local canary rollout using MeshMultiZoneService and MeshHTTPRoute, while keeping the stable pool available across zones.
products:
  - mesh
tldr:
...
prereqs:
  inline:
    ...
    - title: Identity
      content: |
        Keep the **same identity model** across the stable and canary backends before you test this pattern. If you apply `MeshIdentity` to existing workloads, restart those workloads before you test the canary route so they actually serve the new certificate.
next_steps:
  - text: "Global Color Routing"
    url: "/mesh/scenarios/global-color-routing/"
---

The Kong Air engineering team wants to test a new **Baggage Tracking API** in one zone without changing the routing policy everywhere else. This guide shows the current best-practice shape: create global `MeshMultiZoneService` resources on the Global CP, then apply a **zone-local** `MeshHTTPRoute` only in the rollout zone.
...
Create the `MeshMultiZoneService` (MMZS) resources on the **Global Control Plane**. The cleanest pattern is to select the generated `MeshService` objects by their service-name labels.
...
{% warning %}
Scope of validation: the canary *split* above is validated. The cross-zone *failover* below is **not**, it's the right policy family and shape, but we did not confirm it end-to-end. When the stable and canary workloads ran mixed identity models, otherwise-correct HTTP routing failed TLS verification. Validate this step in your own environment after confirming a consistent identity and trust setup, and restart the workloads selected by your `MeshIdentity` resources.
{% endwarning %}
...
That earlier pattern was too optimistic. The safer guidance is:
