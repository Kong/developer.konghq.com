...
The zone token is signed by a signing key that is autogenerated during the first start of the control plane.
Tokens are never stored in the control plane, the only thing that is stored are the signing keys used to verify if a token is valid.
...
* It's allows for easier and more reproducible deployments of the control plane, and it's more in line with GitOps.
...
> Disabling it means that any data plane proxy can impersonate any service.
