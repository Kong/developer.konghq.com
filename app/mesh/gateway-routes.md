...
use [`MeshHTTPRoute`](/mesh/policies/meshhttproute/) and
...
gateway data plane proxies](/mesh/policies/).
Otherwise, [filtering and routing traffic](/mesh/policies/meshhttproute/) is
configured as outlined in the docs.

Note that when using [`MeshHTTPRoute`](/mesh/policies/meshhttproute/) and [`MeshTCPRoute`](/mesh/policies/meshtcproute/) with builtin gateways, `spec.to[].targetRef` is restricted to `kind: Mesh`.
...
along with the following [`MeshHTTPRoute`](/mesh/policies/meshhttproute/) rule, the only one present in the mesh:
...
[`MeshHTTPRoute`](/mesh/policies/meshhttproute/) rules can themselves specify an additional list of hostnames to further
...
If your traffic isn't HTTP, you can use [`MeshTCPRoute`](/mesh/policies/meshtcproute/) to balance traffic
between services.
