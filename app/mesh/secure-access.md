...
Note: this CA is not the same CA used for service-to-service communication.
...
> and [control plane to control plane](#control-plane-to-control-plane-multi-zone) traffic.
...
See [Data plane proxy authentication](/mesh/dp-auth/) and [Zone proxy authentication](/mesh/zone-proxy-auth/).
...
1. Prepare certificates. Generate a TLS pair with a PKI of your choice and store it in PEM-encoded format in `/tmp/tls.crt`, `/tmp/tls.key`. Store the CA used to sign this pair in `/tmp/ca.crt`.
