...
To configure [built-in gateway](/mesh/built-in-gateway/) listeners, use the `MeshGateway` resource.
...
If [transparent proxy](/mesh/transparent-proxying/) isn't set up, you must add the listener explicitly as an outbound to your `Dataplane` objects if you want to access it:
...
In a multi-zone deployment, `MeshGateway` resources must be created on the global control plane. For more information, see [Built-in gateways](/mesh/built-in-gateway/#multi-zone).
