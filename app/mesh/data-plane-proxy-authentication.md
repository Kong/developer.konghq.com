...
{% if_version gte:2.13.x %}
- `mesh` + `workload`: the token is bound to data plane proxies with a specific `kuma.io/workload` label value, providing finer-grained authentication than service tags.
{% endif_version %}
...
If you need to generate a new token for a `Dataplane`, or if you are using service account token projection on Kubernetes, you can configure dynamic token reloading. Set the `kuma-cp` configuration property `dpServer.auth.enableReloadableTokens` to `true`. When enabled, `kuma-dp` detects changes to the token file, reloads the token, and uses the new value when establishing a new connection to `kuma-cp`.{% if_version gte:2.7.x %} By default, `enableReloadableTokens` is enabled on Kubernetes.{% endif_version %}
...
{% if_version gte:2.13.x %}
   * `Workload` (string) — the workload label value for the data plane proxy
{% endif_version %}
