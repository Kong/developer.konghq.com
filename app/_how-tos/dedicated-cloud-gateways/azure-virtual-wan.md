...
tldr:
  ...
  a: |
    Using a Virtual WAN in Azure, you can create a Dedicated Cloud Gateway in {{site.konnect_short_name}} with Azure as the network provider. 
    When the Azure network is `Ready` in {{site.konnect_short_name}}, you can configure Virtual WAN by creating the peering role and assigning it to the service principal. 
    You can use your Azure Dedicated Cloud Gateway after it displays as `Ready` for Virtual WAN.
related_resources:
...
1. Scroll until you see `Ready` for Virtual WAN peering.