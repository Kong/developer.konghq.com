---
title: Configure the OIDC plugin with {{site.identity}}
...
tags:
    - authentication
description: Learn how to configure {{site.identity}} with OIDC.

tldr: 
  q: How do I configure {{site.identity}} with OIDC?
  a: | 
...
faqs:
  - q: How do I use {{site.identity}} with Consumer-scoped plugins and OIDC auth?
    a: |
...
You can configure the [OIDC plugin](/plugins/openid-connect/) to use {{site.identity}} as the identity provider for your Gateway Services. In this example, you'll apply the plugin to the control plane globally, but you can alternatively apply it to the Gateway Service.
...
In this example:
* `issuer`: Setting that connects the plugin to your IdP (in this case, {{site.identity}}). 
...
Access the `example-service` Gateway Service using the short-lived token generated by the authorization server from {{site.identity}}:
