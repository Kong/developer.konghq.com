...
prereqs:
  inline:
    - title: Aggregated MCP server
      content: |
        Before testing with Insomnia, complete the [Aggregate MCP tools from multiple AI MCP Proxy plugins](/ai-gateway/mcp/aggregate-mcp-tools/) guide.  
        That guide produces:
...
After completing the aggregation workflow described in **[Aggregate MCP tools from multiple AI MCP Proxy plugins](/ai-gateway/mcp/aggregate-mcp-tools/)** and exposing an autogenerated or aggregated MCP server, follow this guide to test and validate its MCP tools using Insomnia’s MCP Clients.
