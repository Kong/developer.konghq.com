...
This guide covers:
1. How sticky sessions work in {{site.kic_product_name}}
...
To enable drain support, you must start {{site.kic_product_name}} with the `--enable-drain-support` flag set to `true`. This can be done in your deployment YAML:
...
3. During a deployment or pod termination:
   ...
   - {{site.kic_product_name}} (with drain support enabled) identifies terminating pods
