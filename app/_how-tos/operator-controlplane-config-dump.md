...
permalink: /operator/controlplanes/how-to/config-dump-server/
breadcrumbs:
  - /operator/
  - index: operator
...
curl http://localhost:10256/debug/controlplanes/namespace/default/name/my-controlplane/config/
```
...
```bash
curl -s http://localhost:10256/debug/controlplanes/namespace/default/name/my-controlplane/config/ | jq .
```
...
```bash
curl -s http://localhost:10256/debug/controlplanes/namespace/default/name/my-controlplane/config/ \
...
3. Dump and inspect the configuration:
   ...
   ```bash
   curl -s http://localhost:10256/debug/controlplanes/namespace/default/name/test-cp/config/ | \
...
while true; do
    SIZE=$(curl -s "http://localhost:10256/debug/controlplanes/namespace/$NAMESPACE/name/$NAME/config/" | wc -c)
...
curl -s http://localhost:10256/debug/controlplanes/namespace/default/name/cp1/config/ > cp1.json
curl -s http://localhost:10256/debug/controlplanes/namespace/default/name/cp2/config/ > cp2.json
