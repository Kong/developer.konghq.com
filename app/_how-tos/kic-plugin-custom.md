...
Kong provides a way to deploy custom plugins using both {{ site.kgo_product_name }} and the {{ site.kic_product_name }} Helm chart. This guide shows how to use the Helm chart, but we recommend using {{ site.kgo_product_name }} if possible. See [Kong custom plugin distribution with KongPluginInstallation](#TODO) for more information.
