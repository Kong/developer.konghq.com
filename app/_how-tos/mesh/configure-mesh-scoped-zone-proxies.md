...
1. Confirm each proxy is attributed to the right mesh and has the role you expect:
   ...
   {:.info}
   > Check the pods rather than `kubectl get dataplanes`. A zone proxy's `Dataplane` is generated by the control plane and is not a Kubernetes object, so it does not appear in `kubectl` output. To see it, query the control plane with `kongctl get mesh inspect dataplanes`.
