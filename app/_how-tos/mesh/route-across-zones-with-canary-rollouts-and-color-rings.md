...
content_type: how_to
permalink: /mesh/route-across-zones-with-canary-rollouts-and-color-rings/
...
### Apply the rollout-zone canary route
...
1. Check that the synced MMZS resources matched the expected backends:
...
1. Test from a pod in the rollout zone:
...
1. Apply the following to each zone:
...
1. Verify the pods are running with sidecars injected (2/2 READY):
...
1. Create three MMZS resources on the Global CP:
...
1. After sync, check that each MMZS matched the expected MeshServices:
...
1. From the `grn` caller, confirm every response comes from the green pool:
...
1. On zone1, scale down the blue backend and wait for the pod to terminate:
...
1. Re-run the affinity check from the `blu` caller (a zone1 pod):
   ...
   Traffic continues to return `"color":"blu"`, now served from zone2's blue pool.

1. Restore zone1 when done:
...
Both patterns hold up under direct traffic checks: the canary split lands close to its configured weight, and each color ring only ever reaches its own pool.

1. Confirm the weighted canary split from Pattern 1. From a pod in the rollout zone, send a batch of requests to the stable/canary MMZS hostname and tally how many hit the canary backend versus the total:

   ```bash
   CALLER=$(kubectl get pod -n kong-air-production -l app=passenger-portal -o jsonpath='{.items[0].metadata.name}')
   RESULTS=$(kubectl exec -n kong-air-production "$CALLER" -c passenger-portal -- \
     sh -c 'for i in $(seq 1 50); do wget -qO- http://check-in-api-global.mzsvc.mesh.local:8080; echo; done')
   echo "$RESULTS" | grep -c canary
   echo "$RESULTS" | wc -l
   ```

   Expected output: roughly 10% of the 50 requests resolve to a `check-in-api-canary-*` hostname, matching the `90/10` weight:
...
1. Confirm color-ring pinning from Pattern 2. Callers should only ever reach their own color, never load-balance across colors. From the `blu` caller, loop several requests and collapse the responses to their unique values:

   ```bash
   BLU_POD=$(kubectl get pod -n kong-air-production -l app=check-in-api,color=blu \
     -o jsonpath='{.items[0].metadata.name}')
   kubectl exec -n kong-air-production "$BLU_POD" -c check-in-api -- \
     sh -c 'for i in $(seq 1 20); do wget -qO- http://flight-control-all.mzsvc.mesh.local:8080; echo; done' \
     | sort -u
   ```

   Expected output: a single distinct response, proving every request landed on the blue pool and none leaked to green:
...
1. Repeat from the `grn` caller and confirm it resolves to a single distinct value from the green pool:
...
Together, these confirm both patterns behave as configured: the canary split is proportioned close to its weight, and color pinning is exclusive, a caller in one color ring never receives a response from another color's pool.
