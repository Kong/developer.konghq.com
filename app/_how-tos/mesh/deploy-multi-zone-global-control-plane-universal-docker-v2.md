...
content_type: how_to
permalink: /mesh/deploy-multi-zone-global-control-plane-universal-docker-v2/
breadcrumbs:
  - /mesh/
related_resources:
...
1. Create a `ZoneIngress` resource definition. The `advertisedAddress` and `advertisedPort` are the values other zones use to reach this ingress, so set them to an address that is routable from other zones:
...
1. Open the global control plane GUI at <http://127.0.0.1:5681/gui> and check that:

   * `zone1` appears in the **Zones** view.
   * `zone1-ingress` appears in the **Zone Ingresses** view.
   * The `default` mesh has mTLS enabled.

You can now deploy services into `zone1` by following the [Universal get-started series](/mesh/get-started/universal/install/), starting at the `kv` and `demo-app` steps. To add more zones, repeat the [zone control plane](#start-the-zone-control-plane) and [zone ingress](#start-the-zone-ingress) steps with a new database, zone name, container IP, and set of host ports.

## Clean up

To tear down the deployment, remove the containers, the network, and the temporary directory:
