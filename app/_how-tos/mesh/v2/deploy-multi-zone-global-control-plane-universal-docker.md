...
content_type: how_to
permalink: /mesh/v2/deploy-multi-zone-global-control-plane-universal-docker/
breadcrumbs:
  - /mesh/v2/
related_resources:
...
To manage the deployment with [kumactl](/mesh/v2/cli/), connect it to the global control plane.
...
1. Run the zone control plane:
   ...
   * Port `25678`: the local XDS endpoint that data plane proxies inside the zone connect to.
   ...
   `KUMA_MULTIZONE_ZONE_KDS_TLS_SKIP_VERIFY=true` is required because the global control plane's certificate is self-signed in this demo. For production, use a certificate signed by a trusted CA. See [Secure access across services](/mesh/v2/secure-access/) for more information.
