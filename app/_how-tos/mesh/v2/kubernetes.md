...
{{site.mesh_product_name}} ships with a read-only [UI](/mesh/v2/interact-with-control-plane/) that you can use to view the sidecar proxies connected to the control plane and retrieve {{site.mesh_product_name}} resources. By default, the UI listens on the API port `5681`.
...
For more details, see [Interacting with the {{site.mesh_product_name}} control plane](/mesh/v2/interact-with-control-plane/).

## Introduce zero-trust security

By default, traffic between services is insecure and not encrypted. To encrypt traffic, enable the [Mutual TLS](/mesh/v2/policies/mutual-tls/) policy. The policy provisions a Certificate Authority (CA) that automatically assigns TLS certificates to the injected data plane proxies running alongside the services.
...
{{site.mesh_product_name}} doesn't define default traffic permissions, so no traffic flows with mTLS enabled until you define a [`MeshTrafficPermission`](/mesh/v2/policies/meshtrafficpermission/) policy.
