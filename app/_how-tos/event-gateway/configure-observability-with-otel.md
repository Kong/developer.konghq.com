...
The [`grafana/otel-lgtm`](https://github.com/grafana/docker-otel-lgtm) image bundles Grafana, Tempo, Prometheus, Loki, and a built-in OTEL Collector. No custom configuration files are needed.

Open a new terminal window and create a directory for the Docker Compose file:
```sh
mkdir otel && cd otel
```
...
1. Navigate to **Explore** (compass icon in the sidebar).
...
1. Use the **Search** tab, select `eventgw` as the service name, and click **Run query**.
