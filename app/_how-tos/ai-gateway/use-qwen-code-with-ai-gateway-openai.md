...
* `formats: [type: openai]`: Declares that this model accepts requests in OpenAI-compatible format, matching what {{ site.claude_code }} sends natively, even though the upstream model is OpenAI.
...
* `targets`: Specifies which upstream AI Model Provider model to route requests to. Here, `provider: generic-openai` references the AI Model Provider we created earlier, and `name: gpt-5-mini` specifies which OpenAI model to call upstream.
