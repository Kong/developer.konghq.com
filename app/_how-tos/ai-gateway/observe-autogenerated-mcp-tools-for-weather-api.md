...
permalink: /ai-gateway/observe-autogenerated-mcp-tools-for-weather-api/
...
To observe traffic for MCP tools, you first must **enable logging and statistics** on the AI MCP. Apply the below configuration to reconfigure the entity while preserving the WeatherAPI key in `tools[].query`:
