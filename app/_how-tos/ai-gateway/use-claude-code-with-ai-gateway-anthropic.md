...
Set `llm_format: anthropic` to tell {{site.ai_gateway}} that requests and responses use {{ site.claude}}'s native API format. This parameter controls schema validation and prevents format mismatches between {{ site.claude}} Code and the gateway.
...
This output confirms that {{ site.claude}} Code routed the request through {{site.ai_gateway}} using the `claude-sonnet-4-5-20250929` model we selected while starting the {{ site.claude}} Code session.
